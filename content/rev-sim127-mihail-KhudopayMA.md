...
- Название класса должно быть существительным. "Render" это действия, а "Renderer" это тот, кто выполняет это действие
```java
class Render
...
- Не нужно хранить хэш-код. Он должен рассчитываться при вызове метода `hash()`
```java
private final int hash;
```

- В equals не нужно сранивать хэш-код. Кроме всего, сравнивать хэш-коды бессмысленно- они не гарантируют уникальность
```java
@Override
...
public Entity getEntityByCell(Cell cell) {
  //тут возможно будет(спойлер: будет) еще какая-то общая логика  
  return entities.get(cell);
}
```
...
**7. abstract class Entity и его простые наследники Grass/Tree/Rock**
...
Путь тооже креатура должна находить самостоятельно, а не принимать во входящие.
...
**11. class PopulateMap implements Action**

Помещать существ на фиксированные позиции- это довольно слабо.  
Помещай существ на случайные координаты.

**12. class MoveEntities implements Action**

Много сложной логики перемещения креатур.  
Таким образом, единая ответственность,- выполнение хода,- разделена между двумя классами: этим и Creature(метод `makeMove(...)`).  
...
**13. Дублирование логики в class SpawnGrass и SpawnHerbivore**

Нарушение DRY. Два класса, которые делают одно и то же- заселяют в карту существ определенного вида в заданном количестве.  
Чеклист ТЗ:
```
...
Замечаний меньше, чем обычно. Каких-то грубых ошибок нет.  
