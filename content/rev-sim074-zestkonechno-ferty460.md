...
Этот метод необходим карте для реализации ее едииной ответственности- при всех операциях в карте с участием координат, координату нужно проверять на вхождение в карту.
...
Решение дискуссионно, его плюсы: в разные экземпляры креатур можно внедрять разное поведение.
Минусы: сейчас для одного вида существ декларировано одно поведение (класс Заяц - класс ПоведениеЗайца), в Зайца можно внедрить поведение Волка и наоборот, структура проекта заметно усложняется.
...
Например, если игра будет использовать HttpRenderer, то часть информации будет отображаться по сети, а часть- печататься в консоль.
