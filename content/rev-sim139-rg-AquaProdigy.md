...
Тогда при попытке это сделать, заяц будет удален со своей старой координаты и не записан на новую координатыу  
То есть, заяц просто тихо исчезнет из карты- это глюк.

+ 👍 Ок, пусть будет. Хотя я бы возвращал не мапу, а только `List<T>`
```java
...
protected Speed speed;
```
При использованиии `int`, бросание исключения при отрицательных значениях можно сделать с помощью простого метода.  
