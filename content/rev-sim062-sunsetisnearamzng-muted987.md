...
2. Симуляция делает только по один ход, потом снова нужно вводитб команду для совершения одного хода. 
...
- Сомневаюсь, что соседний узел можно называть "родителем"
```
public class Node {
  Node parent;
  //...
...
**3. По умолчанию** объект-поле класса и так инициализируется null'ом
```
...
```
Другой вопрос- а нужны ли дакие мудреные методы, которые возвращают такие мапы, вообще? Не готов дать авторитетный ответ в рамках этой реализации.
...
- Методы в классе не должен быть статическим. Чем меньше статики в ООП программе, тем лучше. Статические классы должны быть в основном в утилитных классах, а не в классах типа этого
```
