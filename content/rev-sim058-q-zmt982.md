...
Пока еще нет достаточного понимания SRP и принципах правильноого распределения ответственностей, сделай и используй доску только с такими методами
```
public class Board {
...
Если модель хочет что-то сообщить миру, например о том, что существо перемистилось из точки А в точку Б, то модель не должна эту информацию печатать в консоль самостоятельно.
Иначе модель должна будет знать много лишего- в какую среду печатается информация(консоль, windows ui), на каком языке и т.д.
