...
- Перечни в виде массивов, сетов, листов и мап, долны назваться во множественном числе
```java
...
public void removeEntity(Coordinates coordinates) {
  Entity entity = entities.remove(coordinates);  <-- ПЕРЕД УДАЛЕНИЕМ НУЖНО ПРОВАЛИДИРОВАТЬ КООРДИНАТУ
  if (entity != null) {
...
Из-за того, что креатуры чамостоятельно не управляют своим движением, код в этом классе сложен и загадочен
```java
public TurnManager() {
