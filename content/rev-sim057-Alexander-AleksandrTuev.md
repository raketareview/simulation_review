...
1. После запуска симуляции, программа просто распечатывает в консоль несколько десятков кадров карты. 
...
То есть, будет ли координата после сдвига находиться в рамках размеров карты
```
public boolean canShift(Coordinates coordinates, CoordinatesShift coordinatesShift)
```
Этот метод используется в интересах поиска пути, вот там пусть и находится.
...
Например, `act()` вкласс Creature выполняет какие-то действия, а в классе Rock этот же метод имеет пустое тело
```
...
А на самом деле он принимает экземпляр пользовательского класса Map(Карта симуляции): `import com.example.simulation.Map;`.
...
Дублирование хранения координат в существе и карте добавляет сложности, но все равно можно немного упростить с помощью применения стандартного интерфейса java
```
...
**18. Дублирование кода** в классах RespawnHerbivoreAction, RespawnPredatorAction и RespawnGrassAction. Избавься от дублирования по аналогии с предыдущим примером.
...
Самолеты умеют летать, но не умеют плавать, а корабли ровно наоборот.

Нужно создать абстрактный класс Транспорт и два наследника- Корабль и Самолет.

1. Неправильная иерархия классов
...
Потомки реализуют свои умения. А на те методы, которые неумеют делать, ставят пустые заглушки
```
...
2. Правильная иерархия классов

Транспорт в данном случае знает только свою грузоподъемность. Способы перемещения он не знает, это заботы его наследников
```
