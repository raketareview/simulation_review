...
Нарушения есть, но их мсеньше, чем бывает обычно.
...
public Point findNextStep(GameMap map, Point start, Class<? extends Entity> targetType) {
  Point current = queue.poll();
  for (Point shift : SHIFT_POINTS) { // больше никакой говномагии- просто нормальный алгоритм
...
Поиск пути должен происходить в классе поиска пути, а здесь нужно только использовать этот поиск, примерно так:
```java
...
SpawnAction волкSpawnAction = new SpawnAction((point) -> new Трава(), КОЛИЧЕСТВО_ТРАВЫ);
