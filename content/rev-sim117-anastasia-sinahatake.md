https://github.com/sinahatake/Simulation  
[Anastasia]

Есть над чем поработать.

## ХОРОШО

+ 👍 Координаты существ не хранятся в самих существах(мне так больше нравится)
+ 👍 Реализовано пауза/пуск во время работы

## ЗАМЕЧАНИЯ

**1. Нейминг**

- Придерживайся единообразия. пакеты называй или в единственном, или во множественном числе
```Java
package actions;
package entity;
```

- Для избежания путаницы не называй пакеты, классы, интерфейсы так же, как называются классы и интерфейсы стандартной библиотеки.  
Есть стандартный интерфейс `Map`, не должно быть кастомного пакета с таким же названием
```java 
package map;

//ПРАВИЛЬНО:
package gamemap;
```

- Аббревиатуры в названиях лучше писать так же, как и другие слова
```java
class BFSPathFinder

//ЛУЧШЕ:
class BfsPathFinder
```
Например, какое название понятнее: HTTPURL или HttpUrl?

- Избегай неоднозначностей. 

Под "логгированием" в яве понимается нечто отличное от печати в консоль. А этот метод печатает в консоль
```java
void logEntitiesState()

//ПРАВИЛЬНО:
void printEntitiesState()
```
*Oracle Java code conventions, part."Naming conventions"*  
*Мартин, "Чистый код", гл.2*  
*Ютуб, Немчинский "Как называть переменные, методы и классы?"*

**2. Текст в exception** всегда должен быть на английском языке
```java
throw new IllegalArgumentException("Позиция " + pos + " находится за пределами карты.");
```
Исключение это не просто телеграмма, которая летит сквозь слои. 
У exception особое назначение- если исключение вылетит и не будет перехвачено внутри программы, то аварийно прекратит выполнение программы.
Тогда на экране будет распечатано сообщение эксепшена, и это сообщение должно быть понятно сисадмину в любой точке планеты. 
А значит, сообщение должно быть на английском. 

Интерпритация исключения и перевод его на локальный язык должно происходить там, где это соответствует архитектуре программы. 
Или не происходить вовсе, если исключение не планируется перехватывать.

**3. Нарушение конвенции кода**

В любой ситуации выделяй тело блока скобочками, даже если тело состоит из одной строки. 
Исключение- метод equals(), там можно после if не выделять блоки скобочками
```java
if (!isInsideBounds(pos)) return false;
if (map.isInsideBounds(p))
  list.add(p);

//ПРАВИЛЬНО:
if (!isInsideBounds(pos)) {
  return false;
}
if (map.isInsideBounds(p)) {
  list.add(p);
}
```

**4. Форматирование строк**

Если нужно печатать или создавать строку с более, чем одним подстановочным значением или значение вставляется внутрь сообщения, используй форматирование- тогда сразу будет виден весь шаблон
```java
System.out.print(" " + grid[i][j] + " ");

//ПРАВИЛЬНО:
System.out.printf(" %s ", grid[i][j]);
```

**5. Нарушение DRY**, магические буквы, числа, слова. Вводи константы 
```java
System.out.println("Нажмите 'P' для паузы, 'Q' для выхода");
if (cmd.equals("P")) {...}
if (cmd.equals("Q")) {...}

//ПРАВИЛЬНО:
private final static String PAUSE = "P";
private final static String QUIT = "Q";

System.out.printf("Нажмите '%s' для паузы, '%s' для выхода \n", PAUSE, QUIT);
if (cmd.equals(PAUSE)) {...}
if (cmd.equals(QUIT)) {...}
```
*Фаулер, "Рефакторинг", гл.8, "Замена магического числа символической константой"*  
*refactoring.guru "Замена магического числа символьной константой"*  

**6. record Position(int x, int y)**

+ 👍 Нет ничего лишнего, это хорошо. Record для координаты- идеально.

- Избыточно.

Это переопределение не имеет практического смысла- неявняые геттеры в record'е работают абсолютно так же
```java
public record Position(int x, int y) {
  @Override
  public int x() {
    return x;
  }

  @Override
  public int y() {...}
}

//ПРАВИЛЬНО:
public record Position(int x, int y) {
}
```
Вообще, рекорд нужно использовать как что-то простое и самодостаточное. 
Если на `record` хочется навесить дополнительный функционал в виде методов, то нужно подумать- а не стоит ли тогда вместо него сделать обычный `class`.

**7. class WorldMap**

- Избыточно.

Нет необходимости иметь две хешмапы, которые хранят записи координата-существо и существо-координата.
Достаточно иметь только хешмапу координата-существо
```java
private final Map<Position, Entity> occupancy = new HashMap<>();  <-- ЭТОГО ДОСТАТОЧНО
private final Map<Entity, Position> locations = new HashMap<>();  <-- ЭТО ИЗБЫТОЧНО
```
Две хешмапы могут улучшить время выполнения некоторых операций. Но они усложняют код и делают класс более громоздким.  
В таких случаях нужно выбирать простоту.  
Преждевременная оптимизация является антипаттерном.

- Нарушение SRP, методы чужих ответственностей. 

Карта должна только хранить существа и обеспечить базовые операции с ними: вставить, выдать одно существо и список всех хранимых существ, удалить.
Если какой-то метод не нужен для обеспечения этого функционала, значит он не принадлежит к ответственности карты, а принадлежит к чужой ответственности.
Здесь методы чужих ответственностей:  
Подсчитать количество существ определенного типа, вернуть всех креатур, определить что по координате находится существо нужного класса.

Наверное, для проекта в целом полезно иметь метод, который считает существ нужного класса. 
Но этот процесс не имеет никакого отношения к единой ответственности карты- хранению существ в себе.  
Для организации хранения/удаления/выдачи существ, карте не нужно считать зайцев и волков.  

Методы чужих ответственностей должны находиться в тех классах, в интересах которых они работают.
Если один и тот же метод используют разные классы, то метод нужно вынести в отдельный класс, например, BoardUtils.

- Никогда не возвращай null
```java
private final Map<Entity, Position> locations = new HashMap<>();

public Position getPositionByEntity(Entity entity) {
  return locations.get(entity);  <-- может вернуть null
}
```
Возврат null повышает риск возникновения NullPointerException в программе.  
*Мартин, "Чистый код", гл.7.7-8*  
*Ютуб, Немчинский "Почему нельзя возвращать NULL?"*

- При попытке удалить суслика, которого нет, нужно кидать исключение
```java
public void removeEntity(Position pos) {
  Entity entityToRemove = occupancy.get(pos);
  if (entityToRemove != null) {
    occupancy.remove(pos);
    locations.remove(entityToRemove);
  }
}
```
Сейчас при попытке удалить существо по несуществующей координате, ничего не произойдет.  
И сначала это может показаться хорошим выбором- программа не крашится и продолжает работать. 

Но в том-то и дело, что попытка удалить несуществующее существо означает баг алгоритма- значит, программа работает некорректно. 
Эту ситуацию нужно максимально быстро выявить и исправить.  
Иначе, в программе будет самый страшный баг- плавающий, который будет то проявляться во время работы программы, то нет.

+ 👍 Хорошо, что возвращается копия мапы, а не оригинал. Тем самым соблюдается инкапсуляция и защищается внутреннее устройство класса
```java  
private final Map<Position, Entity> occupancy = new HashMap<>();

public Map<Position, Entity> getOccupancy() {
  return new HashMap<>(occupancy);
}
```

+ 👍 Хорошо, что при добавлении существа, проверяется координата- находится ли она в пределах карты или нет.

- Нарушение SRP, OCP. 

Карта должна работать со всеми хранимыми существами одинаково и не работать как-то по-особому с конкретным классом представителей и не должна даже знать по именам наследников Entity
```java
public Set<Creature> getAllCreatures()

//ПРАВИЛЬНО ТАК:
public Set<Entity> getAllEntities() //этот метод уже есть в карте- пусть клиент отбирает отсюда креатур

//ИЛИ ТАК:
public List<? extends Entity> getAllBy(Class<? extends Entity> clazz)  //вернуть существ определенного класса 
```
Если карта будет знать по именам наследников Entity и иметь для них персональные методы, то класс будет открыт для изменений.  
Например, при добавлении в проект класса Птица, понадобится изменить класс Карта и добавить в него метод `getAllBirds()`.

- Метод совершения хода в карте- нарушение SRP.
Теперь можно вызвать из карты метод хода и телепортировать зайца из одного края в другой минуя все правила игровой логистики
```java
Карта карта = new Карта(100, 100);
Заяц заяц = new Заяц();
карта.putEntity(new Координата(0, 0), заяц);
карта.moveEntity(заяц, new Координата (99, 99));

/* class Карта */
public void moveEntity(Entity entity, Position newPos) {
  //17 строк логики
}
```
Должна ли карта учитывать логистику зайцев? Если да, то как?

Здесь метод телепортации частично пытается ответить на этот вопрос- выполняет какие-то проверки. 
Тем самым Карта, фактически, берет на себя часть общеигровой логики- хранит часть правил совершения хода существами.  
Единственно правильный ответ- карта вообще не должна иметь в себе метод телепортации.

Напоминаю, что единая ответственность карты- хранить существ и обеспечивать к ним доступ.  
Правила перемещения существ по карте к этому не относится.

- Сразу кастуй
```java
if (entity instanceof Creature) {
  creatures.add((Creature) entity);
}

//ПРАВИЛЬНО:
if (entity instanceof Creature creature) {
  creatures.add(creature);
}
```

**8.interface PathFinder**

+ 👍 Интерфейс поиска пути это хорошо. Теперь можно делать разные реализации поиска: BFS, AStar и т.д.

+ 👍 Сигнатура метода поиска- идеально
```java
List<Position> findPath(Position start, Class<? extends Entity> targetType, WorldMap map);
```

**9. class BFSPathFinder implements PathFinder**

- Мапа координата-координата обычно при реализации поиска используется как эрзац-заменитель связного списка
```java
Map<Position, Position> parent = new HashMap<>();
```
Вместо мапы координата-координата сделай класс связного списка, алгоритм тогда будет выглядеть понятнее.
Например, так:
```java
class Node {
  private Coordinates coordinates; 
  private Node parent;
  //...
  public Coordinates getCoordinates() {...}
  public Node getParent() {...}
}
```

- Особые случаи обрабатывай сразу
```java
LinkedList<Position> path = new LinkedList<>();

while (!cur.equals(start)) {
  //...    
  if (cur == null)
    return Collections.emptyList();
}

return path;

//ПРАВИЛЬНО:
if(cur == null) {
  return Collections.emptyList();
}

LinkedList<Position> path = new LinkedList<>();

while (!cur.equals(start)) {
  //...    
}

return path;
```

**10. abstract class Entity**

- Нарушение SRP, зависимость модели от представления- существо хранит спрайт с собственным изображением
```java
public abstract String getSymbol();
```
Модель(а это модель) не должна зависеть от представления и знать, как ее будут показывать юзеру.  
Потому что в разных средах(консоль, Swing, Android) одна и та же модель может быть показана разными способами- пиксельной картинкой, анимацией etc.  
Спрайты всех существ должны храниться в классе, который распечатывает карту.

**11. abstract class Creature extends Entity**

- Нарушение конвенции кода.

Конструктор должен стоять первым из методов. 
Иначе это вводит читателя в заблуждение- он подумает, что у класса есть только конструктор по умолчанию. 
А тут конструктора по умолчанию как раз и нет.

- Неправильное использование интерфейсов.

В проекте есть интерфейс PathFinder. А значит, может быть сколько угодно его реализаций(BFS, A*).  
Если креатура содержит в себе поиск, то она не должна самостоятельно выбирать себе реализацию этого поиска, а должна принимать ее в конструктор.  
Это сделает класс универсальным и он сможет использовать любой поиск
```java
public abstract class Creature extends Entity {
  private final PathFinder pathFinder = new BFSPathFinder();

  public Creature(int hp, int speed) {...}
  //...
}

//ПРАВИЛЬНО:
public abstract class Creature extends Entity {
  private final PathFinder pathFinder;

  public Creature(int hp, int speed, PathFinder pathFinder) {...}
  //...
}
```

- Креатура должна сама знать, что для нее является едой. Иначе клиент может заставить Волка есть Траву. Или камни
```java
public void makeMove(WorldMap map, Class<? extends Entity> targetType) 

//ПРАВИЛЬНО:
private final Class<? extends Entity> food;

public void makeMove(WorldMap map) 
```

- Процедурный прием.

Пара значений x/y составляют координату, использовать эти переменные по отдельности- процедурный прием.  
Использовать как объект координат- ООП прием
```java
private void makeRandomMove(WorldMap map) {
  List<int[]> dirs = new ArrayList<>(Arrays.asList(
    new int[]{1, 0},
    new int[]{-1, 0},
    new int[]{0, 1},
    new int[]{0, -1}
  ));
  //...
  for (int[] d : dirs) {
    Position np = new Position(pos.x() + d[0], pos.y() + d[1]);
    //...
  }  
}

//ПРАВИЛЬНО:
private final static List<Position> DIRECTIONS = List.of(new Position(1, 0), ...);

private void makeRandomMove(WorldMap map) {
  //...
  for (Position direction : DIRECTIONS) {
    Position np = sum(pos, direction);
    //...
  }  
}
```

- Эти значения должны быть синхронизированы между собой
```java
public abstract class Creature extends Entity {
  private int satiety = 10;

  public void makeMove(WorldMap map, Class<? extends Entity> targetType) {
      //...
      satiety = 10;
  }
}

//ПРАВИЛЬНО:
private final static int MAX_SATIETY = 10;
private int satiety = MAX_SATIETY;

public void makeMove(WorldMap map, Class<? extends Entity> targetType) {
  //...
  satiety = MAX_SATIETY;
}
```

**12. class Predator extends Creature**

Не забывай про инкапсуляцию
```java 
public class Predator extends Creature {
  int attackDamage;
  //..
  public Predator(int hp, int speed, int attackDamage) {...}
}

//ПРАВИЛЬНО:
private final int attackDamage;
```

**13. interface Action**

👍 Идеально 
```java 
public interface Action {
  void execute(WorldMap map);
}
```

**14. class SpawnUtils**

- Это не утилитный класс. Это Manager или Helper.

Утилитные классы должны быть `final` и иметь приватный конструктор.  
Не должно быть возможности унаследоваться от утилиты или сделать ее экземпляр.  
*Блох, "Java. Эффективное программирование", изд.3, гл.2.4*

- Неправильные действия.

Null возвращать нельзя. Если что-то идет не так- бросай исключение
```java 
public Position getValidRandomPosition(WorldMap map) {
  //...  
  System.err.println("WARN: Не удалось найти свободное место для спавна.");
  return null;
}
```
Если по какой-то причине метод не может выполнить свой контракт, он должен бросить исключение.  
Вызывающий код должен перехватить исключение и обработать.

**15. class SpawnAction implements Action**

+ 👍 Всё ок. 

- Сюда можно еще вынести поиск случайной координаты из `SpawnUtils`- он существует только в интересах этого класса.

**16. class MoveAllCreaturesAction implements Action**

В целом ок, но этот класс не должен брать на себя чужую ответственность и знать, какую еду едят креатуры
```java
if (entity instanceof Creature creature) {
  Class<? extends Entity> targetType = null;
  if (creature instanceof Predator) {
    targetType = Herbivore.class;
  } else if (creature instanceof Herbivore) {
    targetType = Grass.class;
  }
  creature.makeMove(map, targetType);
}

//ПРАВИЛЬНО:
if (entity instanceof Creature creature) {
  creature.makeMove(map);
}
```

Иначе, среди прочего, класс будет открыт для изменений и при развитии проекта будет постоянно изменяться:
```java
if (creature instanceof Bird) {
  targetType = Butterfly.class;
}
```

**17. class ConsoleRenderer**

- Избыточно.

Нет необходимости сначала делать массив с пустыми картинками, потом заполнять их спрайтами существ, а потом этот массив распечатывать.  
Среди прочего это плохо тем, что при больших картах это будет сильно забивать память.  
Например, если карта будет 100*100 ячеек, то придется на каждом ходе записывать в память 10.000 спрайтов
```java
public void printMap(WorldMap worldMap) {
  
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
       grid[i][j] = "⬜"; <-- формирование
    }
  }

  for (Map.Entry<Position, Entity> entry : worldMap.getOccupancy().entrySet()) {
    //формирование
   }

  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
       //печать
    }
  }
}
```

Печатай карту по ходу ее чтения:
```java
public void printMap(WorldMap worldMap) {
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
       Position position = new Position(x, y);
       if(worldMap.isSquareEmpty(position)) {
        System.out.print(EMPTY_SPRITE);
       } else {
        Sprite sprite = worldMap.get(position).getSymbol();
        System.out.print(sprite);
       }
    }
    System.out.println();
  }
}
```

- Спрайты существ должны храниться здесь, а не браться из самих существ.

И тут ты задашь вопрос:  
Так если спрайты существ хранить здесь, а не в самих существах, то при добавлении нового существа в проект, придется изменять этот класс.  
Значит ли это, что класс открыт для изменений и тем самым нарушает OCP?

Да, класс будет открыт для изменений в том смысле, что при добавлении в проект нового существа Bird, нужно будет в рендерер внеси изменение- добавить спрайт для Bird.  

Но в данном случае это норм- единая ответственность Рендерера как раз и состоит в том, чтобы печатать карту.  
А для этого рендерер должен знать все, что касается распечатки, в том числе- изображения существ.  
SRP гласит: "У класса должна быть только одна причина для изменения". Здесь эта причина- изменение визуала.

Спрайты существ нельзя хранить в самом существе потому, что изображение зависит от среды, в которой нужно показывать существо.  
Но где-то спрайты хранить надо. Поэтому хранить надо в том классе, который осуществляет взаимодействие с представлением.  
То есть, здесь.

Для отработки понимания этого пункта, можно сделать такое упражнение:  
Создать два Main'а. Один из них должен запустить симуляцию, где существа на карте будут изображаться в виде эмодзи.  
А второй должен запустить симуляцию, где существа на карте будут изображаться в виде букв: P - predator, r - rock и т.д., а земля- точкой.  
Тогда станет понятно, что хранить спрайты в самих существах- не вариант. 

**18. class Simulation**

- Нарушение DI. Класс не должен инициализировать сам себя. 
В данном случае класс инициализирует сам себя тем, что сам создает карту, а не принимает ее в конструктор
```java
public class Simulation {
  private final WorldMap map = new WorldMap(10, 10);
  //...
}
```
Таким образом нельзя создать несколько игровых конфигураций с разными картами.  
*Мартин, "ЧК", гл.11, "Отделение конструирования системы от ее использования"*

- Неправильно использование экшенов.

Экшены должны запускаться без предварительных условий и использоваться одинаково через полиморфизм. 
Идея экшенов состоит в том, чтобы можно было быстро изменить функционал, добавляя/удаляя экшены.
Если какой-то экшен нужно запускать по предварительному условию, значит его нужно переделать так, чтобы условие проверялось внутри самого экшена
```java
public void nextTurn() {
  moveAllCreaturesAction.execute(map);
  if (map.numberOfEntityOfType(Grass.class) < 1) {
    new SpawnAction(Grass::new, 5).execute(map);
  }
  if (map.numberOfEntityOfType(Predator.class) < 1) {
    new SpawnAction(() -> new Predator(10, 1, 1), 5).execute(map);
  }
  //...
}

//ПРАВИЛЬНО:
private final static int RESPAWN_LEVEL = 0;

private final List<Action> turnActions = List.of(
  new MoveAllCreaturesAction(),
  new RespawnAction(Grass::new, 5, RESPAWN_LEVEL),
  new RespawnAction(Tree::new, 5, RESPAWN_LEVEL),
  //...  
);

public void nextTurn() {
  for(Action action : turnActions) {
    action.execute(map);
  }
  //...
}
```

- Магические числа
```java
new SpawnAction(() -> new Predator(10, 1, 1), 5).execute(map);
```

- Этот метод не инициализирует init экшены, он их выполняет. Выполнять init экшены можно сразу в конструкторе
```java
private void initSpawnActions() {
  new SpawnAction(Grass::new, 5).execute(map);
  new SpawnAction(Tree::new, 5).execute(map);
  //...
}

//ПРАВИЛЬНО:
public class Simulation {

private final List<Action> initActions = List.of(
    new SpawnAction(Grass::new, 5),
    new SpawnAction(Tree::new, 5),
    //...  
  );

  public class Simulation(WorldMap map, ...) {
    this.map = map;
    //...
    executeActions(initActions);
  }

  private void executeActions(List<Action> actions) {
    for(Action action : actions) {
      action.execute(map);
    }
  }
}
```

**19. class Main**, содержит точку входа main

👍 Только создает и запускает Симуляцию, это хорошо.

## ВЫВОД

В целом, замечаний меньше, чем обычно.  
Посмотреть ролики Немчинского про SOLID.

n.117(265)  
#ревью #симуляция 