...
Если нужно передать смысл "близко, рядом", то есть более однозначные слова, напр. "near". 
...
+ 👍 При БОЛЬШИНСТВЕ операций с участием координаты, предварительно выполняется валидация.  
...
```
А то на вопрос "Свободна ли координата +100500, -100500" метод ответит что да, свободна.  
Тогда как правильный ответ- такой координаты в карте вообще нет.
...
+ 👍 С точки зрения SOLID, это превосходный класс.  
В нём нет ничего лишнего и есть всё, что надо.
...
public class CostNode extends Node{
  //Дополнительные поля стоимости хода- для A*
}
```
...
Сейчас вместо этого заинтересованные классы `Simulation` и `WorldMapConsoleRenderer` обращаются к `ConsoleLock.LOCK` как к ГЛОБАЛЬНОЙ ПЕРЕМЕННОЙ.  
