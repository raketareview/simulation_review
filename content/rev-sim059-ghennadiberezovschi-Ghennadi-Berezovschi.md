...
Идея Action состоит в том, что нужно создать семейство родственных классов, объединенных общим интерфейсом.
...
Я бы сделал три майна:
  * Запуск симуляции через конфигурирование
  * Запуск дефолтной симуляции с емоджи рендерером
  * Запуск дефолтной симуляции с буквенным рендерером
