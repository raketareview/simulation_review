...
- Да, но почему `n`? Зжесь лучше бы смотрелось `en` или `entity`- не вижу смысла конкретно здесь делать отднобуквенную переменную
```
...
- Принцип группировки здесь классов по пакетам неясен. В пакете `statics` находятся классы неходячих существа(Трава, Камень, Дерево), и Карта. Почему карта находится в одном пакете с существами, совершенно неясно.
В то же время, ходячие существа находятся в другом пакете- `dinamics`.

- Пакет `service` содержит классы, которые нельзя отнести к сервисам, например, `Actions`, `Renderer`, `BFS` etc. Очевидно, что под "сервисом" здесь понимается просто класс, который просто что-то как-то делет.  
...
Сделовательно, единая ответственость по поиску пути размазывается между несколькими классами: один класс ищет конкретный экземпляр еды, а класс поиска прокладывает путь к этой еде.  
...
Возможно, если будешь еще делать консольные пет-проекты, для подобных случаев лучше сделать универсальные диалоги.  
