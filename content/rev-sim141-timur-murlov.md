...
+ 👍 Используется паттерн Listener/Callback для сообщений о производимых событиях
...
В английском языке сначала стоит подлежащее(тут опускаем), потом сказуемое(глагол), потом- дополнение.
...
- Ввод при использовании консоли уже подразумевает нажатие энтера
```java
...
подсчитать количество существ определенного вида с одновременным уменьшением их количества на единицу,  
подсчитать количество существ определенного вида с одновременным увеличением их количества на единицу,  
вернуть случайную пустую координату.

Наверное, для проекта в целом полезно иметь метод, который считает количество существ и одновременно уменьшает их количество.  
...
И тем более- одновременно уменьшать их количество.
...
Значит клиент эту составляющую ответственность крты берет на себя и вытягивает существо через заднюю дверь- используя `getEntities()`.
...
```
Если количество аргументов больше 3-4, то нужно как-то упрощать сигнатуру. Например, передавать объект-аргумент.  
*"ЧК", гл.3, "Объекты как аргументы"*
...
И конкретный синглтон будет создат в зависимости от того, какой из этих методов будет запущен первым
```java
...
```
Это для сделано экономии ресурсов, чтобы разные классы могли пользоваться одним и тем же экземпляром `Random instance`?  
Если да, то так стало хуже- проект без необходимости стал сложнее.
...
//ПРАВИЛЬНО:
System.out.println("Для начала/паузы симуляции введите Enter. Для выхода нажмите 'q' и введите Enter.");
```
