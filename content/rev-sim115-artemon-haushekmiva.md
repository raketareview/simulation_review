...
Не все модели в проекте лежат в этом пакете. Например, эти модели находятся в других пакетах: Coordinates, BFS etc.
...
Преждевреиенная оптимизация является антипаттерном.

- При всех операциях с участием координаты(добавить, выдать, удалить и т.д.), нужно проверять координату на корректность.  
Если координата некорректна(находится вне пределов карты), нужно бросать исключение
```java
public boolean isCellEmpty(Coordinates cell) {  <-- Что скажет, если придет Coordinates.EMPTY?
...
- Работай с координатой целиком, а нее запчастями
```java
...
Для этого классу вообще не нужно знать про существование самого класса `WorldMap` и вспомогательных ему классов.
...
- Нарушение конвенции кода. Вспомогательный приватный методо не должен стоять выше главного по отношению к нему публичного метода.
