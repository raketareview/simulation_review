...
И WorldMap тоже станет моделью, когда он будет переделан и избавлен от представления.
...
Simulation - нечто вроде контроллера
...
Этот класс не содержит путь, он содержит много каких-то публичных методов.  
...
Это то, что касается классических поисков.

Если же тут используется оригинальная концепция, когда призрак охотится за "своим" кладоискателем, 
то в поиск нужно передавать конкретный объект, к которому нужно проложить путь:
```java
public class CustomPathFinder {
  public List<Coordinates> find(Карта карта, Coordinates from, Entity target) {...}
}
```
...
а энтити [пишет в консоль](https://github.com/raketareview/simulation_review/blob/master/content/rev-sim064-davidtagirov-DavidTagirov.md). 
...
public static void main(String[] args) {
  ...
  coinHunter.initDeath(rockCell, worldMap);  //дистанционно убить камень используя кладоискателя в качестве транглюкатора
  System.out.println();
...
Имеется ввиду, что при этом потоки будут управлять классом Simulation, а не наоборот.
...
ООП сильно поплыло на этапе реализации поиска пути.  
Проработай тему SOLID, прежде всего SRP.  
