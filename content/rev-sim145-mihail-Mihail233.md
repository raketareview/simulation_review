...
С точки зрения ООП, моделями так же являются Cell, Node.  
...
то лучше не используй в своей программе такие термины.  
...
Для проекта в целом полезно иметь метод, который находит соседние кдетки.  
Но этот процесс не имеет никакого отношения к единой ответственности карты- хранению существ в себе.  
Для организации хранения/удаления/выдачи существ, карте не нужно искать соседние кдетки.
...
public static void main(String[] args) {
  ...
  coinHunter.initDeath(rockCell, worldMap);  //дистанционно убили камень используя охотника за монетами в качестве дестроера
  System.out.println();
...
Если в экшене кроме `execute(...)` есть другие публичные поля или меитоды, то это не экшен.
```java
...
for (int indexRow = worldMap.getOriginWorldMap().getY(); indexRow < worldMap.getMaxWidthField(); indexRow++) {...}
```
Потому что "Row" это строки, а "WidthField" это ширина поля.  
...
Но здесь решил подрезать публичный досту там, где безусловно нужен.  
Мотивация неясна
```java
public class Main {
...
**Совет:**  
Для тренировки и понимания некоторых ООП концепций, создай два майн-класса.  
Один пусть запускает игру так, как это сейчас- диалолг с юзером и конфигурирование им карты.  
Второй майн пусть запускает программу без диалога с юзером и с параметрами по умолчанию: картой 10x10 и т.д.
