...
- В этом классе публичным должен быть только один метод `findPath(...)`. Если будет несколько публичных методов, то правила пользования поиском будут излишне сложными, как сейчас.
Сейчас, фактически, есть два вида поиска: "дальний поиск" `findPath(...)` и ближний поиск
```
public <T extends Entity> Position findNearestEntity(Position start, Class<T> targetType) {...}
```
