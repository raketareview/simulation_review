...
- Какая цель достигается этим названием, объяснить, что делает метод? Нет
```
...
Например, здесь есть класс `GameManager` и в нем есть такое поле
```
/**
...
```
Инструкция получитась слишком длинной и плохо читаемой, потому что в одну инструкцию ты объеденил несколько инструкций.
...
В ТЗ указано более конкретное требование к классу- это класс, который являет из себя сущность "Карта"
```
...
Крнечно, этот класс не является Картой. Это свалка методов, как-то связанных с Картой- сущностью, которая в этой реализации не представлена в виде класса.
Но это не отменяет того, такое размещение спавнера не позволяет делать разные карты с разными комбинациями существ на них. 
...
Ну так это потому что в реализации нет чётких(да вообще никаких) границ ответственностей.
...
```
Этот "билдер" реализован в тех классах, которые создаются. Но в этом классе, который их создает. 
...
```
Если хочется сделать такие закрученные конструкции, значит творческая мысль пошла не туда. 
...
Если на уровне Creature хочется сделать Билдер, который будет делать своих потомков(идея сама по себе спорная), 
то корректный билдер будет выглядеть так:

а. Сделать конструктор Creature и его наследников protected или rivate. 
...
**?. class InitAllEntities extends Init**
...
**16. class TurnMove extends Turn**

Сейчасв Creature нет метода `makeMove()`, хотя он там должен быть по ТЗ. В принципе, почему бы и нет.
...
**17. Миллион других экшенов**

В программе есть еще миллион других экшенов: TurnEat, TurnPathfinder, TurnDamage etc. 
Все они занимаются какими-то мелкими делами, а их колличество подпадает под критерии антипаттерна "Дробление".
...
Потому что сами они ничего не делают, а только вызывают методы из очередной свалки методов `GameManager`.
...
- Божественный класс, нет внятных границ ответстсвенностей. 
...
**20. class Renderer implements IRenderable**
...
**22. class Simulation implements ISimulation**
...
public Simulation(IRenderable renderable) {
  //...  
...
**23. class App**, содержит точку входа main
...
Нет никакой принципиальной разницы между сборником процедур, которые в программе на Си хранятся в отдельном заголовочном файле.
...
В программе есть большое количество интерфейсов. Большая вполне нормальные, а другие *в рамках текущей реализации* не имеют никакого смысла.
...
```
Вообще ничем. И в первом и во втором случае, если захочется использовать другую реализацию рендерера, придется менять код класса Simulation.
...
Если просто так, потому что это "выглядит объектно-ориентированно", значит архитектуру ты не улучшил, а ухудшил.

III. Нарушение ТЗ

Сергей и так упростил задачу тем, что провел первичную декомпозицию и описал ее в ТЗ. 
Ты нарушил ТЗ и ухудшил архитектуру.
...
Любое твое альтернативное решение по определению будет хуже.

## ВЫВОД

Не видно понимания того, что такое объектно-ориентированный анализ/проектирование/декомпозиция.

Для начала откажись от создания в подобных программах классов Менеджер/Хэлпер/Утилиты. 
...
Вот здесь это более-менее понятно описано: "Применение UML и шаблонов проектирования", Ларман.
