...
- В пакете "config" хранятся класс, который трудно отнести к конфигам: `class UserInput`. 
Это инпутер, через которым происходит диалог с юзером, но никак не конфиг.
...
Если карта зачем-то знает по именам Entity, значит карта делает что-то не то и берет на себя часть чужой логики. 
