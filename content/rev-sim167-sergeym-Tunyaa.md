...
И таких методово-существительных в проекте много:
```java
...
Для организации хранения/удаления/выдачи существ, карте не нужно среднюю позицию между существом и целью.  
...
- Не делай параллельные системы координаты
```java
...
В проекте должен использоваться только один вид координат- тот класс, который содержит точки положение по осям x,y (row, column).  
...
**?. class StraightPathFinder implements PathFinder**

Недостатки этого класса проистекают из его интерфейса.

**?. Интерфейсы в моделях**
...
Чисто формально- почему бы и нет. Но это нарушет ТЗ проекта, а это не очень хорошо.  
...
**?. class Herbivore extends Creature implements Eatable, Attackable**
...
Есть и плюсы- интерфейс программы выглядит красиво 👍  
