...
- Координата описывает положение точки в пространстве, а не прямоугольник
```
...
Потому что сейчас при вызове метода `getNeighbors(...)` каждый раз пересоздается объект(а массив это объект) со смешщениями. 
...
Эта цель должна быть передана в аргументы метода поиска, как я указал в замечаниях к `interface Pathfinder`.
...
Использовать для BFS ноду от А* конечно можно, но выглядеть будет это несколько коряво- нода A* хранит в себе лишнюю информацию для BFS.  
...
Например, когда у карты спрашивают существо по какой-то координате,а там пусто- вот тогда геттер чтобы не возвращать null может вернуть экземпляр `EmptyArea`.  
...
- Первоначальное заполнение карты статичными существами и заглушками "Пустое мето", про которые я писал выше. 
...
public class RespawnAction implements Action {
  ...
  int final int minAmount;
  int final int maxAmount;

  public AddingGrassAction(MapWorld mapWorld, Class type, int minPercent, int maxPercent) {
    //...
...
Например, в проекте есть интерфейс Рендерера, который распечатывает карту. 
Этот рендерер Симуляция должна не самостоятельно инициализировать, а получать в конструктор, чтобы можно было подставлять его любую реализацию
```
...
Потому что при этом неизбежно ухудшается архитектура, но за расширенным функционалом это становится не так заметно.
А приоритет здесь все-таки состоит именно в отработке построения хорошой архитектуры.
