...
Раз пошла тенденция использовать константные классы/классы настроек, нужн осветить тему более подробно.
...
По форме это будет немного оличаться от классического константного класса, но суть останется той же и недостатки будут те же самые
```
public class Main {
  public static void main(String[] args) {
...
Делаем структуру для хранения настроек
```
public class AppSettings {
...
public class InputAppSettingsFactory implements AppSettingsFactory {
  ...
  public AppSettings get() {
    ...
    settings.boardWidth = input("Ширина карты", failMessage, 3, 99);
