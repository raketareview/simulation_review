...
Введите количество травы: 5
Введите количество травоядных: 555
Введите корректное количество: <= 5
Введите количество травоядных: 5
Введите скорость травоядных до 50: 3
Введите здоровье травоядных до 100: 3
Введите количество хищников: 3
Введите скорость хищников до 50: 3
Введите силу атаки хищников до 100: 3
```
...
```
В этой строке 151 символ. Это абсолютно нечитаемая строка, это не круто. 
Круто это когда код понятен. Вводи поясняющие переменные.  
...
Нет никаких оснований для того, чтобы ограничивать Карту квадратной формой, нужно ввести ширину и высоту.
...
Вместо прямого чтения из констант, необходимые данные класс должен принимать в конструктор
```
public Entity getEntityAt(Coordinates coordinates) {
  return map.getOrDefault(coordinates, Constants.NOTHING);
}
...
А как этот путь будут использовать и как при этом использовании будет задействован параметр `int speed` из класса `Creature`- явно не зона ответстсвенности алгоритма поиска пути.

- Почему 10? Почему 14? Магические числа нужно объяснять
```
int G = (i == 0 || k == 0) ? 10 : 14;
```
...
Нормальный костыль: `Nothing` не должен возвращать фейковые координаты, при попытке их спросить должен бросать исключение. Например:
```
public class Nothing extends Entity {
  public Nothing() {
    super(-1, -1);
...
И тогда вместо привязки к константному классу, можно будет юзать заглушку напрямую
```
//БЫЛО:
public class GameMap {
  //...  
...
- Неправильное использование константного класса: чтение данных из него напрямую вместо получеия необходимых данных в конструктор
```
...
```
Ближайшая аналогия: на дороге сиидит кошка, мы к ней подходим и даем пинка- кошка сама побежит, нам не придется лично переставлять ее лапы или указывать как именно она должна передвигаться и что при этом делать. 
...
Меню и управление стоп/пуск нужно вынести в отдельный класс.
