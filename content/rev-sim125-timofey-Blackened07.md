...
private void initializeInitActions() <-- НУ ХОТЬ ТАК
```
...
И в Карте для этой цели нужно использовать хешмапу- она как раз и должна хранить сявзку координата-существо самым оптимальным образом.

Это универсальный класс для использования в Карте для хранения и для построения пути в Поиске пути?  
Тогда это класс, который совмещает в себе две ответственности и таким образом нарушает SRP.  
Аналогия из реального мира- универсальный пылесос-кофеварка. Сделать можно, но никто не купит.
...
private final Cell[][] map = new Cell[HEIGHT][WIDTH];
```
Таким образом, положение ентити на карте дублируется двумя способами: через координату в `Cell` и через положение ячейки(Cell) в ячейке массива(сорян за товнологию).
...
Определить, что по координате находится Creature/Herbivore/etc.  
Вернуть случайного оккупанта.  
...
Условие должно считываться сразу, а не пересчитываться при чтении кода каждый раз заново в голове
```java
if (new Random().nextInt(100) < 15) {...}
...
```java
public interface TargetChecker {
    boolean isTarget(Class<? extends Entity> clazz);
}
```
...
- Нарушенеи ТЗ. Не вижу здесь метода `makeMove(...)`, указанного в ТЗ
```java
Creature #
...
Сначала нужно сделать экшены в соответствии с духом ТЗ, а потом только их обозревать.
...
Ацкий ад. Что здесь проиходит, трудно описать словами. Это следствие всех предыдущих ошибока рхитектуры.
...
```
Не знаю, что делает этот метод- все названия тут такие, что это без просто невозможно понять это без изучения контекста использования.  
