https://github.com/Blackened07/simulationMap  
[Тимофей]

В целом, удручающе.

## ХОРОШО

+ 👍 Спрайты существ не хранятся в самих существах
+ 👍 Координаты существ не хранятся в самих существах(мне так больше нравится)
+ 👍 Реализовано пауза/пуск во время работы

## ЗАМЕЧАНИЯ

**1. Нейминг**

- Для избежания путаницы не называй пакеты, классы, интерфейсы так же, как называются классы и интерфейсы стандартной библиотеки.  
Есть стандартный интерфейс `Map`, не должно быть кастомного пакета с таким же названием
```java
package map;

//ПРАВИЛЬНО:
package simulationmap;
```

- Избыточно. Приписка "Data" тут является словом-паразитом, потому что оно не добавляет ничего нового к пониманию того, что делает класс
```java
class CoordinatesData

//ПРАВИЛЬНО:
class Coordinates
```

- Не сокращай без необходимости. Желание, чтобы оба названия были одной длины- недостаточноая необходимость
```java
private final int col;
private final int row;

//ПРАВИЛЬНО:
private final int column;
private final int row;
```

- Хуже венгерской ноттации только венгерская ноттация, которая обманывает.
Эта паременная не является реализацией интерфейса Map, это массив
```java
private final Cell[][] map = new Cell[HEIGHT][WIDTH];

//ПРАВИЛЬНО:
private final Cell[][] cells = new Cell[HEIGHT][WIDTH];
```

- Так метод возвращает много единиц травы или одну единицу? 
```java
public Map<Cell, Grass> getGrasses() {
  return grass;
}

//ПРАВИЛЬНО:
public Map<Cell, Grass> getGrasses() {
  return grasses;
}
```

- Название должно объяснять, что делает метод. 

Этот метод вставляет траву в карту. 
Но умалчивает о том, что вставляет не просто так, а по какой-то хитрой логике
```java
public void setGrass(CoordinatesData coords) {
  //...
  if (new Random().nextInt(100) < 20) {
    cell.setOccupant(new Grass());
    //...
  }
}

//ПРАВИЛЬНО:
public void setGrassКраткоеОписаниеУсловий(CoordinatesData coords)
```

- Название обманывает. Это просто инкремент, а не инкремент после еды
```java
public void increaseSatietyLevelAfterEating() {
  this.satietyLevel++;
}
```
Я этот метод могу вызвать после еды, перед едой, вместо еды- факт предварительного поедания никак не влияет на возможность пользоваться этим методом.

- Название обманывает. Метод ищет путь, а не ближайший ресурс
```java
public class PathFinder {
  public static List<CoordinatesData> findNearestResource(...) {...}
}

//ПРАВИЛЬНО:
public class PathFinder {
  public static List<CoordinatesData> find(...) {...}
}
```

- Списки должны называться во множнственном числе
```java
private final List<Action> initAction;

//ПРАВИЛЬНО:
private final List<Action> initActions;
```

- Для избежания путаницы, не называй поле так же, как и метод
```java
private final List<Action> initAction;
//...

private void initAction() {
  initAction.add(...);
}

//ПРАВИЛЬНО:
private final List<Action> initActions;
//...

private void initializeInitActions() <-- НУ ХОТЯ БЫ ТАК
```

- Геттер должен что-то возвращать. Геттер, который ничего не возвращает- нонсенс
```java
void getActions()
```

- Методы нужно писать стилем camelCase
```java
private void PlanActions()

//ПРАВИЛЬНО:
private void planActions()
```
*Oracle Java code conventions, part."Naming conventions"*  
*Мартин, "Чистый код", гл.2*  
*Ютуб, Немчинский "Как называть переменные, методы и классы?"*  


**2. Приоритет- понятность кода, а не экономия строк**

Вводи поясняющие переменные, делай программу более понятной
```java
creatures.put(getCell(coords.getCol(), coords.getRow()),
  getCreature(coords.getCol(), coords.getRow()));

//ПРАВИЛЬНО:
Cell cell = getCell(coords.getCol(), coords.getRow();
Creature creature = getCreature(coords.getCol(), coords.getRow());
creatures.put(cell, creature);
```

**3. Если в блоке if есть return**(break, continue, throw, exit и т.д.), то else не пишется.
 
В этом случае неважно, будет else или нет, так как программа будет работать одинаково, а код без `else` будет выглядеть читабельней
```java
if (randomNum < 50) {
 return new Earth();
} else if (randomNum < 75) {
  return new Grass();
}

//ПРАВИЛЬНО:
if (randomNum < 50) {
 return new Earth();
} 
if (randomNum < 75) {
  return new Grass();
}
```

**4. Нарушение DRY**, магические буквы, числа, слова. Вводи константы. А если они уже есть- пользуйся 
```java
private static final String INTRO = "Добро пожаловать в симуляцию!\n" +
   "Для старта симуляции введите \"S\"\nДля паузы введите \"P\"\nДля остановки симуляции введите \"E\"\nДля выхода из программы введите \"exit\"";
private static final String START_COMMAND = "s";
private static final String PAUSE_COMMAND = "p";
private static final String STOP_COMMAND = "e";
private static final String EXIT_PROGRAM = "exit";

//ПРАВИЛЬНО:
private static final String START_COMMAND = "s";
private static final String PAUSE_COMMAND = "p";
private static final String STOP_COMMAND = "e";
private static final String EXIT_PROGRAM = "exit";

private static final String INTRO = """
   Добро пожаловать в симуляцию!
   Для старта симуляции введите '%s'
   Для паузы введите '%s'
   Для остановки симуляции введите '%s'
   Для выхода из программы введите '%s'
   """.formatted(START_COMMAND, PAUSE_COMMAND, STOP_COMMAND, EXIT_PROGRAM);

```
*Фаулер, "Рефакторинг", гл.8, "Замена магического числа символической константой"*  
*refactoring.guru "Замена магического числа символьной константой"*

**5. class CoordinatesData**

+ 👍 Нет ничего лишнего, это хорошо.

+ 👍 Хорошо, что поля координаты называются row/column а не y/x. Мне так больше нравится.  
В этом случае нет, как часто бывает, путаницы с порядком расположения в массивах: координата(x,y), а в массивах наоборот- массив[y, x].

- Класс может быть преобразован в record без потери функционала.

**6. class Cell**

Забегая вперед, этот класс здесь лишний.  
Класс хранит в себе координату, базовый энтити(тут он называется оккупант) и однонаправленный связный список
```java
public class Cell {

private final CoordinatesData coordinates;
  private Occupant occupant;
  private Cell parentCell;
  //...
}
```
Я не могу понять, какую сущность из себя представляет этот класс.  

Это node однонаправленного списка для Поиска пути?  
Тогда в этом классе достаточно иметь координату и ссылку на предка.

Это сущность для хранения ентити по координате?  
Тогда ссылка на предка это лишнее. 
И в Карте для этой цели нужно использовать хешмапу- она как раз и должна хранить связку "координата-существо" самым оптимальным образом.

Это универсальный класс и для Карты(хранение существ) и для Поиска пути(связный список)?  
Тогда это класс, который совмещает в себе две ответственности и таким образом нарушает SRP.  
Аналогия из реального мира- универсальный пылесос-кофеварка. Сделать можно, но никому такой не нужен.

**7. class SimulationMap**

- Карта содержит в себе фиксированные размеры, поэтому становится неуниверсальной- нельзя создать игровые конфигурации с картами разных размеров
```java
public final static int WIDTH = 12;
public final static int HEIGHT = 12
```
Фиксированные размеры могут быть у шахматной доски(8x8) или игры крестики-нолики(3x3).  
То есть в тех играх, где размер игрового поля жестко фиксирован правилами.  
Здесь размеры должны приниматься в конструктор, чтобы можно было создавать карту произвольных размеров.

- Избыточно сложное, неоптимальное хранение существ.

Карта хранит не существ, а рассмотренные ранее "ячейки", которые содержат существ
```java
private final Cell[][] map = new Cell[HEIGHT][WIDTH];
```
Таким образом, положение ентити на карте дублируется двумя способами: через координату в `Cell` и через геометрическое положение ячейки(Cell) в ячейке массива(сорян за товтологию).

Хранение существ в массиве избыточно и нарушает чеклист ТЗ
```
Проблемы и ошибки в коде:  ...Неоптимальный выбор коллекции для хранения состояния ячеек
```
Здесь имется ввиду, что хранить существ в массиве- неоптимально. Хранить существ в хешмапе- оптимально.

Для хранения разных типов существ используются разные коллекции
```java
private final Map<Cell, Creature> creatures = new ConcurrentHashMap<>();
private final Map<Cell, Grass> grass = new ConcurrentHashMap<>();
```
Это нарушение чеклиста ТЗ
```
Проблемы и ошибки в коде:
Класс Map ...параллельные коллекции для разных типов существ (для каждого типа своя коллекция)
```

Для хранения существ в Карте должна использоваться только одна коллекция, вот такая:
```java
private final Map<Coordinates, Entity> entities = new ConcurrentHashMap<>();
```

- Нарушение SRP, методы чужих ответственностей. 

Карта должна только хранить существа и обеспечить базовые операции с ними: вставить, выдать одно существо и список всех хранимых существ, удалить.
Если какой-то метод не нужен для обеспечения этого функционала, значит он не принадлежит к ответственности карты, а принадлежит к чужой ответственности.
Здесь методы чужих ответственностей: определить, что по координате находится Creature/Herbivore/etc, вернуть случайного оккупанта и т.д.

Наверное, для проекта в целом полезно иметь метод, который возвращает случайного оккупанта. 
Но этот процесс не имеет никакого отношения к единой ответственности карты- хранению существ в себе.  
Для организации хранения/удаления/выдачи существ, карте не нужно уметь искать случайного оккупанта.  

Методы чужих ответственностей должны находиться в тех классах, в интересах которых они работают.
Если один и тот же метод используют разные классы, то метод нужно вынести в отдельный класс, например, SimulationMapUtils.

- Карта не должна сама себя заселять существами. 
Иначе она становится не универсальной и нельзя будет создать несколько конфигураций игры с разными начальными комбинациями существ на карте
```java
private void fillMap() {
  //...  
  map[row][col] = new Cell(new CoordinatesData(col, row), getRandomOccupant());  <-- НАЧАЛЬНОЕ ЗАСЕЛЕНИЕ СУЩЕСТВ НА КАРТУ
}

private Occupant getRandomOccupant()  <-- СОЗДАЕТ СЛУЧАЙНОЕ СУЩЕСТВО ДЛЯ ЗАСЕЛЕНИЯ
```
Объект не должен сам себя инициализировать.  
*Мартин, "ЧК", гл.11, "Отделение конструирования системы от ее использования"*

- Нарушение DRY. Методы делают одно и то же- помещают в карту существ определенного типа
```java
public void setGrass(CoordinatesData coords) {
  Cell cell = getCell(coords.getCol(), coords.getRow());
  if (new Random().nextInt(100) < 20) {
    cell.setOccupant(new Grass());
    grass.put(getCell(coords.getCol(), coords.getRow()),
    getGrasses(coords.getCol(), coords.getRow()));
  }
}

public void setCarnivore(CoordinatesData coords) {
  Cell cell = getCell(coords.getCol(), coords.getRow());
  if (new Random().nextInt(100) < 15) {
    cell.setOccupant(new Carnivore());
    creatures.put(getCell(coords.getCol(), coords.getRow()),
    getCreature(coords.getCol(), coords.getRow()));
  }
}
```
Общий код выноси во вспомогательные методы.

- Магические числа выноси в константы
```java
if (randomNum < 50) {
 return new Earth();
} else if (randomNum < 75) {
  return new Grass();
}
```

- Нарушение инкапсуляции, класс предоставляет клиентскому коду подробности внутреннего устройства и доступ к нему
```java  
public Map<Cell, Grass> getGrasses() {
  return grass;
}
```
В данный момент клиентский код может выполнять любые операции с HashMap Карты напрямую, игнорируя допустимые картой способы. 
Например так
```java
Карта карта = new Карта();
<заселить карту существами>
карта.getGrasses().clear(); //геноцид- удаление из карты всех существ, минуя дозволенные картой механизмы
```
Из-за этого класс является гибридом со всеми вытекающими последствиями: *"Чистый код", гл.6*

Тут есть два варианта, как сделать правильно:  
Либо вернуть не оригинал мапы, а ее копию: 
```java
public Map<Cell, Grass> getGrasses() {
  return new HashMap(grass);
}
```

Либо возвращать существ персонально:
```java
public Entity get(Coordinate coordinate)
public Coordinates getCoordinates(Entity entity)
```
Лично мне больше нравится второй вариант, когда существ возвращают персонально. 

- Не используй сложную логику в аргументах условных операторов.  
Условие должно считываться сразу, а не пересчитываться каждый раз заново в голове при чтении кода 
```java
if (new Random().nextInt(100) < 15) {...}

//ПРАВИЛЬНО:
int randomNumber = new Random().nextInt(100); 
if (randomNumber < 15) {...}
```

- Нарушение OCP. 

Карта должна работать со всеми хранимыми существами одинаково и не работать как-то по-особому с конкретным классом представителей и не должна даже знать по именам наследников Entity
```java
public Map<Cell, Creature> getCreatures() {...}
public Map<Cell, Grass> getGrasses() {...}

//ПРАВИЛЬНО ТАК:
public List<Entity> getAll() //и тогда пусть клиент отбирает отсюда креатур

//ИЛИ ТАК:
public List<? extends Entity> getAllBy(Class<? extends Entity> clazz)  //вернуть существ определенного класса 
```
Если карта будет знать по именам наследников Entity и иметь для них персональные методы, то класс будет открыт для изменений.  
Например, при добавлении в проект класса Птица, понадобится изменить класс Карта и добавить в него метод `getBirds()`.

❌ Класс собрал в себе худшие практики для класса Карта.


**8. class PathFinder**

- Соблюдай требования утилитных классов.

Это утилитный класс, он содержит только статические методы. 
Утилитные классы должны быть `final` и иметь приватный конструктор.  
Не должно быть возможности унаследоваться от утилиты или сделать ее экземпляр.  
*Блох, "Java. Эффективное программирование", изд.3, гл.2.4*

- Это утилитный класс, а не контроллер. Поэтому не должен находиться в пакете "controller".

- Как пользоваться методом поиска- неясно.

На первый взгляд, все просто. 
Есть метод поиска, в который передается карта, точка старта и класс для определения цели
```java
public static List<CoordinatesData> findNearestResource(SimulationMap map, CoordinatesData startCoords, TargetChecker targetChecker)
```
Но этот таргетчеккер суперзагадочен
```java
@FunctionalInterface
public interface TargetChecker {
  boolean isResource(SimulationMap map, CoordinatesData startCords);
}
```
Чтобы понять, как это все должно работать в целях поиска пути, нужно долго ковыряться в коде.

Я бы понял, если бы определитель цели на самом деле определял цель, например, вот так:
```java
public interface TargetChecker {
    boolean isTarget(Entity entity);
}
```
Тогда было бы понятно, как пользоваться поиском и что он бы делал в этом случае:  
Искал в карте путь от точки старта до ближайшей сущности, которая targetChecker'ом распознается как цель.

Еще проще, если существо ест только один вид еды, передовать сразу класс еды как цель
```java
public List<Coordinates> find(SimulationMap simulationMap, Coordinates start, Class<? extends Entity> target) {
  //ищет путь на карте simulationMap от точки start
  //до точки, где находится существо нужного класса(напр. target == Grass.class)
}
```

**9. enum OccupantType**

Енам существует для дополнительной типизации классов
```java
public enum OccupantType {
  CARNIVORE, HERBIVORE, GRASS, ROCK, EARTH, TREE
}
```
Не используй дополнительную типизацию классов, потому что это дублирует стандартные способы.  
Используй стандарные способы определять принадлежность объектов к классам:
```java
String name = entity.getClass().getSimpleName();

switch (name) {
  case "Herbivore": //...
  case "Predator":  //...
}
```

**10. abstract class Occupant**

- Нейминг.

По сути, это класс Entity из ТЗ:
```java
Entity 
Корневой абстрактный класс для всех существ и объектов существующих в симуляции.
```
Почему ты решил его переименовать в оккупанта, мне неясно.
Внимательно читай ТЗ и соблюдай его, это необходимый навык в работе программиста. 

Термин:
```
Оккупант — это тот, кто участвует в оккупации, то есть насильственном временном захвате и контроле территории другой страны или ее части вооруженными силами.
```
Может ли трава быть оккупантом? Нет, трава не служит в вооруженных силах.

Если это название дано для лулзов, то не надо пошутеек в коде- коллег это будет сильно раздражать.  

- Не нужно использовать дополнительную типизацию
```java
public abstract OccupantType getType();
```
Используй стандартные способы определения принадлежности объектов к типам: `instanceof`, `getClass()`.

- Общие данные потомков храни в их предке.

Если все потомки должны сообщать свой дополнительный тип, это общая информация всех потомков.  
Значит эти данные должны храниться на уровне общего предка
```java
public abstract class Occupant {
  public Occupant() {}
  public abstract OccupantType getType();
}

//ПРАВИЛЬНО:
public abstract class Occupant {
  private final OccupantType occupantType;

  public Occupant(OccupantType occupantType) {
    occupantType.this = occupantType;
  }
    
  public OccupantType getType() {...};
}
```
Да, использовать доп.типизацию вообще нельзя. 
Но я сейчас говорю о другом- о способе хранения общей информации в иерархии классов.

**11. abstract class Creature extends Occupant**

- Нарушение Low Coupling, неправильное использование Action.

```java
public abstract Action planAction(SimulationMap map, CoordinatesData startCords);
```
Существа не должны знать про классы Action'ы и как-то их использовать. 
Подробнее экшены рассмотрим позже, пока просто тезис.

- Нарушение ТЗ. Не вижу здесь метода `makeMove(...)`, указанного в ТЗ
```java
Creature #
Абстрактный класс, наследуется от Entity ...Имеет метод makeMove() - сделать ход.
```

**12. class Carnivore extends Creature**

- Нарушение дизайна ООП. 

При создании экземпляра класса, значения обязательных полей предка должны устанавливаться через конструктор, а не сетиться  
```java
ublic class Carnivore extends Creature {

  private final int attackPower;

  public Carnivore() {
    this.attackPower = 5;
    setCurrentHealth(15);
    setSatietyLevel(6);
  }
  //...
}

//ПРАВИЛЬНО:

ublic class Carnivore extends Creature {
  private static final int HEALTH = 15;
  private static final int SATIETY_LEVEL = 6;
  private static final int ATTACK_POWER = 5;

  private final int attackPower;

  public Carnivore() {
    super(HEALTH, SATIETY_LEVEL);
    this.attackPower = ATTACK_POWER;
  }
  //...
}
```
*Эккель "Философия Java", гл.5, "Конструктор гарантирует инициализацию"*

**13. interface Action**

Действие должно совершаться над миром(картой), а не над менеджером экшенов
```java
public interface Action {
  void perform(ActionManager actionManager);
}

//ПРАВИЛЬНО:
public interface Action {
  void perform(SimulationMap simulationMap);
}
```
ТЗ по этому поваду говорит следующее:
```
Action - действие, совершаемое над миром. Например - сходить всеми существами. 
...Каждое действие описывается отдельным классом и совершает операции над картой.
```

**14. Про классы-реализации Action**

- Идея `Action` здесь не осмыслена. 

Идея состоит в том, что нужно создать семейство родственных классов, объединенных общим интерфейсом.  
Каждый из этих классов должен делать что-то свое с картой: одна акция должна заселять карту существами, другая делать ходы и т.д.  
Action'ы, изложенные в ТЗ, это вариант реализации паттерна Command. 

На первый взгляд, с экшенами у тебя все в порядке- есть классы `GlobalMoveAction` и `EatingAction`.

Но эти классы не самодостаточные- они по сути являются просто клиентами божественного класса `ActionManager`.  
А класс `ActionManager`, в свою очередь, это просто сборник функций, сделанный в стиле процедурного программирования.  
В этом классе собраны методы, которые решают разные задачи над картой и контроллером(?!)
```java
public class ActionManager {

  private final MapController controller;
  private final Map<OccupantType, EatingRules> eatingRules = new EnumMap<>(OccupantType.class);

  public void moveCreature(CoordinatesData start, CoordinatesData dest) {...}

  public void eating(CoordinatesData creatureCoords, CoordinatesData destCoords) {...}

  public void fillCreatures() {...}

  public void fillGrass() {...}

  public void checkEntitiesValue() {...}
}
```

Смысл Action'ов по ТЗ состоит в том, что должен быть общий класс/интерфейс Action и его наследники.  
В каждом экшене должен быть только один публичный метод(не публичных может быть сколько угодно).  
Это вариация паттерна Command- экшены должны быть родственны и одинаково использоваться через полиморфизм. Примерно так
```java
interface Action{
 void execute(Карта карта);
}

class ХодитьAction реализует Action {
  void execute(Карта карта) {
    //обойти всю карту
    //найти каждую креатуру
    //и дать ей пинка чтоб побежала
  }
}

class ДатьСигаретуВсемЗайцамAction реализует Action {
  void execute(Карта карта) {
    //обойти всю карту
    //найти всех зайцев и дать им по сигарете 
  }
}

List<Action> actions = List.of(new ДатьСигаретуВсемЗайцамAction(), new ХодитьAction());
for(Action a: actions) {
  a.execute(карта);
}

/*
Результат: программа обойдет всю карту, найдет всех креатур и даст им пинка, чтобы они побежали.
А каждому зайцу предварительно даст сигарету.
*/
```

Рассматривать подробно классы-экшены и `ActionManager` я не вижу смысла.  
Их недостатки это следствие неправильного подхода к созданию Action'ов.  
Сначала нужно сделать экшены в соответствии с духом ТЗ, а потом только можно будеть их обозревать.

- Нарушение ТЗ. Отсутствует класс-экшен для начального заселения карты существами
```
initActions - действия, совершаемые перед стартом симуляции. Пример - расставить объекты и существ на карте.
```

**15. class MapController**

Просто еще какой-то сборник функций в процедурном стиле.

**16. class Renderer**

Ацкий ад. Что здесь проиходит, трудно описать словами. Это следствие всех предыдущих ошибок архитектуры.

**17. class Simulation**

Здесь должна быть только общая логика управления программой.  
Например
```java
//СЕЙЧАС ТАК:
void nextTurn() {  
  actionsQueue.clear();  <-- ШО ТУТ И НИЖЕ ВООБЩЕ ПРОИСХОДИТ?
  PlanActions();
  checkEntitiesValue();
  getActions();

  controller.getCreatures().clear();
  controller.getGrass().clear();

  actionManager.fillCreatures();
  actionManager.fillGrass();
}

//ДОЛЖНО БЫТЬ ПРИМЕРНО ТАК:
void nextTurn() {  
  for(Action action : turnActions) {  //выполнить все турн-экшены
    action.perform(simulationMap);
  }
  renderer.render(simulationMap);  //распечака карты на каждом ходе
}
```

В этом классе должна происходить работа на уровне высоких абстракций: вызов экшенов, рендерера и т.д.  
Вот такие ошметки более низкой логики должны находиться не здесь, а в соотвествующих классах
```java
private void PlanActions() {
  controller.getCreatures().keySet()
    .forEach(cell -> {
    Creature creature = controller.getCreatures().get(cell);
    ensureDeath(creature, cell);
    actionsQueue.add(creature.planAction(controller.getMap(), cell.getCoordinates()));
    setSatietyToAllCreatures(creature);
    setHealth(creature);
 });
}
```
Не знаю, что делает этот метод- все названия тут такие, что просто невозможно понять это без изучения контекста использования.  
Но это явно подробность какой-то более низкой логики, которая должна находиться в соотвествующем для нее экшене.

## АРХИТЕКТУРА

Код тяжело читается из-за спагетти кода и неправильной архитектуры, в основе которой находится класс `Cell`.  
Есть тенденция к использованию процедурного стиля- группирование кода в божественные классы типа `ActionManager` и `MapController`.

## ВЫВОД

Переделать программу, придерживаясь ТЗ.  
Подробнее разберись с Action'ами, посмотри ролики на ютубе про паттерн "Command", чтобы получить о нем общее представление.

Для лучшего понимания декомпозиции ООП посмотреть ролики Сергея про шахматы.  
Посмотреть ролики Немчинского про SOLID.

n.125(276)  
#ревью #симуляция 