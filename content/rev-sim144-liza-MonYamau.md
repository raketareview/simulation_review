...
- Это константа.  
И даже по стандартам конвенции не только Oracle, но и Google, здесь должна быть написана UPPER_SNAKE-
...
- Не дублируй имя классов в названии их методов
```java
...
- Расположение класса вызывает вопросы. Сейчас он находится в пакете "service".  
Но я бы сказал, скорее тяготеет к карте и лучше ему было бы находиться в пакете "gamemap".
...
Наверное, для проекта в целом полезно иметь метод, который находит все пустые координаты в карте.  
...
Если программа не сделана в архитектуре, в которорой предусмотрены сервисы, то лучше так не называть пакеты и классы.
...
Впрочем, в этом особого смысла нет, потому и без этой проверки при null-координате произойдет исключение, но чуть ниже.  
Поэтому лучший вариант здесь- не использовать дополнительных проверок.  
Если в алгоритме будет баг, то быстрое падение в этом месте и так само по себе произойдет
```java
public void makeMove(GameMap gameMap) {
  Optional<Coordinates> currentCoordinates = gameMap.getCoordinatesOf(this);
  Coordinates currentCell = currentCoordinates.get();  <-- если в Optional  null, то здесь вылетит NoSuchElementException
  //выполнение хода
}
```
...
Он уже принимает карту, поэтому может ухнать ее размеры непосредственно из нее
```java
public class GameMapRenderer {
...
Сейчас этот класс в себе дергает за хвост какие-то потоки.
```java
...
Конструктор создает объект и предназначен только для инициализации данных, а не для выполнения алгоритмов.  
