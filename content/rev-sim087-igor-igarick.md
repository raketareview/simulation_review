...
```
То есть, поиск спрашивает например, у Волка, по каким поверхностям(существам) Волк может пройти и если на координате стоит такое существо или ячейка пуста(entity по координате == null), то добавляет эту координату в список проходимых
```
