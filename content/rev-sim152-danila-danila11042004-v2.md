...
Публичные и приватные методы могут располагаться вперемешку только тогда, когда публичный метод и его вспомогательные методы идут одной связкой.  
...
+ 👍 Соблюдается инкапсуляция, метод возвращает не оригинал мапы, я ее копию
```java
private final Map<Coordinates, Entity> locationWithEntities;
