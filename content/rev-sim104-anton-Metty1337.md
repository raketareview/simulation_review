...
- Название вводит в заблуждение. Этот енам не храниит типы существ, это фабрика существ
```
enum EntityType
...
👍 Утилитные классы здесь используются правильно: они `final` и имеют приватный конструктор.
...
- При вставке существ в карту, программа вылетает:
```
public class Main {
  public static void main(String[] args) {
...
- Поиск принимает точку начала пути и существо, к которому нужно проложить путь
```
...
- Не передавай `null` в методы и конструкторы, даже если они в состоянии принять null. 
Поля класса итак по умолчанию инициализируется null'ом
```
...
- Не вижу в необходимости существовании этого класса. 
...
Хороший класс, который потенциально может отделать логигу от представления. 
...
Если ты ввел в проект специальный ПринтерСообщений, то вообще вся печать должна идти через этот принтер. А не печатать напряму в консоль, как сейчас:
```
