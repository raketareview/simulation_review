...
- Карта создается с фиксированными размерами, которые берутсчя из констант.  
...
```
Но это- одно из последствий неправильного нейминга карты игры.    
...
- Всегда явно указывай область видимости и `final`, если значения поля не меняется. Да, при использовании ломбока- тоже
```java
@Getter
@Setter
...
Но тогда алгоритм ходьбы Herbivore и Predator буде отличаться?  
Да, будет.  
...
Сейчас `Actions` у тебя это божественный класс, который в себе содержит методы, которые нужны своим наследникам не все сразу
```java
