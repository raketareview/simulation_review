...
```
Но это лишь одно из последствий неправильного нейминга карты игры.    
Вот так было бы лучше:
```java
public List<Coordinates> findPath(Coordinates start, Coordinates target, GameMap gameMap, Class<? extends Entity> targetType)
```
...
```
Фактически, Заяц, как и Волк, делает кусь, но кусь зайца безвреден.  
...
Базовый экшен дылжен быть примерно такой:
```java
...
На самом деле экшены нужно использовать не индивидуально, а группой и дажее использовать через полиморфизм.
Примерно так:
```java
