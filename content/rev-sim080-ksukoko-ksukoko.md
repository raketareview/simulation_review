...
+ 👍 Сигнатура ок, по сигнатуре ясно, как работает метод. Но `build` я бы заменил на `create`- слово "build" в контексте java уж сильно ассоциируется с процессом сборки проекта
```
...
- Синтаксис явы позволяет делать разные извращения с циклом `for`. Но ты не делай- используй цикл в виде `foreach`, либо в виде `for` c тремя позициями.  
