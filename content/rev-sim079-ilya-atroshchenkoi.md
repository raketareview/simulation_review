...
- Нарушение SRP. Метод, кооторый создает список соседних координат
```
public List<Position> getAdjacent()
```
Возмложно, проекту в целом оцень важно знать про соседние координаты, но самой коорденате это не нужно для выполнения ее единой ответственности- идентификации точки в пространстве.  
...
Наверное, для проекта в целом полезно иметь метод, который врозвращает случайую пустую координату. 
...
Как только креатура перемещается или умирает, она дергает карту через соответствующие интерфейс `MovementListener` и `DeathListener`, которе имплементирует карта. 
...
- Нарушение инкапсуляции, все методы публичные. Публичными должны быть те методы, которые предусмотрены для исрользование клиенским кодом.  
Вспомогательные методы должны быть не публичными. Например, этот метод- вспомогательный и не его использование клиентом не предполагается, он должен быть не публичным: `public void findPathToTarget()`.
...
Это попытка использовать паттерн Listener (он же Observer, он же Publish/Subscribe).  
