...
В ТЗ указаны классы с названием "Action", поэтому пакет должен называться "action", а содержащиеся в нем классы- Action'ми. 
...
Action'ы должны быть родственны и одинаково использоваться через полиморфизм.  
...
Рефакторинг пошел на пользу- сейчас программа выглядит значительно лучше, чем в первой версии.  
