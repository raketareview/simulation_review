...
boolean a = true;
```
Однобуквенные названия допустимы в качестве названий счетчиков циклов.  
В остальных случаях- нет.
Давай переменным осмысленные имена, котокрые будут объяснять назначение переменной.

- Апогей бессмысленных однобуквенных названий
```java
...
Этот метод помещает существо на случайную пустую координату.  
...
```
Если поиск пути анализирует существ и подбирает для них еду, значит поиск занят чем-то совершенно не тем.  
...
Здесь 8(ВОСЕМЬ) инструкций скручены в одну. Понять, что здесь происходит- в принципе невозможно
```java
((Herbivore) worldMap.whoHere(steps.getLast())).newHp(hit(((Creature) worldMap.whoHere(steps.getLast()))));
```

Понять это условие- в принципе невозможно
```java
