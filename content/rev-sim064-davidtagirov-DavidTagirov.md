...
❌ Совершенно удивительный класс в реверсном относительно положительного смысле. Содержит всего 3 метода. Из них к ответстсвенности карты имеет отношение только `setEntities(...)`.
...
- Нарушение SRP, рендерер должен только распечатывать данные, но не формировать данные, в данном случае рендерер ведет подсчет ходов
```
...
- Здесь нужно запускать редактор карты, получать ее и передавать в конструктор Симуляции.
