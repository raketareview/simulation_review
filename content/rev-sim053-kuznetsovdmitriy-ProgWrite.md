...
Венгерская ноттация: Entity[] arrayEntities, List<Entity> listEntity, int iAge, String stringName и т.д.
...
-В замечаниях к прошлой верссии программы я писал про другое. Здесь методы сами должны кидать исключения в случае некорректной координаты, 
а не пытаться перехватывать какие-то другие исключения.
Я имел ввиду, что нужно делать вот так
```
public void removeEntity(Coordinates coordinates) {
  if(!isCoordinatesValid(coordinates)) {
...
**5. interface Eatable**  
Интерфейс, который сообщает, что его носитель может быть съеден. А может и не быть съеден
```
...
-Непонятно, зачем интерфейс существует в таком виде. У него только один метод- ввести режим съеденности. То есть, setEaten(true) - существо съедено, setEaten(false) - существо не съедено. 
Ок, а где метод "сообщить, съеден ли он"?

Какую бы проблему мог решать такой интерфейс?  
Например я мог бы из списка всех наличных ентити смапить список всех съедобных существ, которые могут быть съедены:
из List<Entity> в List<Eatable>. Далее прошелся бы по списку съедобных и спросил у них, съедены ли они. 
Например, через метод boolean isНахожусьВсъеденномСостоянии().
...
for(Eatable съедобный: съедобныеСущества) {
  if(съедобный.isНахожусьВсъеденномСостоянии()) {
...
класс Заяц() реализует Съедобность{
  ...
  //...
  @Переопределить 
  public boolean isНахожусьВсъеденномСостоянии() {
    return hp <= 0;
  }
}
```
...
-Содержит координату. Но координата нужна только тому существу, которое ходит, а трава тут не ходит. Поэтому entities должен хранить координату только начиная с уровня Creature.
...
```
Поиску должно быть достаточно знать класс цели- Class<T extends Entity> target. Поиск пути до цели должен происходить одинаково вне зависимости от интересанта поиска- будь то заяц, черепашка или колобок.
