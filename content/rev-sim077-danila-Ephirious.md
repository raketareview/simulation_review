...
Корректно ответить на него только "да" или "нет" возможно только в том случае, если респонтент их дейсвительно употреблял.  
Если не употреблял, то правильный по букве ответ "нет"(не делал этого- а значит и не бросал делать это) будет неверным по его сути.
...
+ 👍 Метод норм. Если не хочешь, чтобы idea в данном методе не подчеркивала предупреждение при касте(либо другой непроверяемой операцией), можно поставить соответствующую аннотацию
```
...
- Эта переменная означает не проценты(PERCENT), а коеффициент. Например, при карте 10х10, в ней будет создано 10 камней(я проверял): 10 * 10 * 0.1 = 10.
Поэтому или переименуй на коеффициент, или проценты сделай процентами
```
//КОЭФФИЦИЕНТ:
...
Трава может быть жива, креатуры могут быть живы- возможно, эти существа должны имплементировать какой-то общий интерфейс, который будет характеризовать их "живое" либо "не живое" состояние.
...
Первая крайность это hard программирование, когда в программе минимум абстракций и настраиваимых значений. Типичный пример, когда карта симуляции может быть только одного фиксированного размера.  
Вторая крайность- soft программирование, когда в программе максимум абстракций и настраиваемых значений. Это делает программу супергибкой но такой же супернепоянтной. 
...
В данном случае, экземпляр фабрики хищника создает класс `InitCommandsFactory`, он же хранит в себе константы для инициализации фабрики. Это те самые данные, которые транзитом идут в конструктор хищника.  
