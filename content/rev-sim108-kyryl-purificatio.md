https://github.com/purificatio/simulation  
[Kyryl]

В целом, норм.

## НЕДОСТАТКИ РЕАЛИЗАЦИИ

1. Я ввел карту 100*100 и запустил бесконечную симуляцию- программа надолго задумалась
```
Введите желаемое количество колонок: 100
Введите желаемое количество строк: 100
```
Введи ограничения на размеры карты, при которых программма будет работать корректно.

## ХОРОШО

+ 👍 Спрайты существ не хранятся в самих существах
+ 👍 Координаты существ не хранятся в самих существах(мне так больше нравится)
+ 👍 Реализовано пауза/пуск во время работы
+ 🚀 Меню для создания карт разного размера

## ЗАМЕЧАНИЯ

**1. Нейминг**

- Если в проекте есть класс `GameMap`, то все переменные с именем, включающим это название, должны быть экземплярами этого класса, либо хранить экземпляры этого класса
```
private final Map<Cell, Entity> cellEntityGameMap = new HashMap<>();
private final Map<Entity, Cell> entityCellGameMap = new HashMap<>();

//ПРАВИЛЬНО:
private final Map<Cell, Entity> cellWithEntities = new HashMap<>();
private final Map<Entity, Cell> entityWithCells = new HashMap<>();
```

- Избыточно
```
GameMap.mapHeight
GameMap.mapWidth

//ПРАВИЛЬНО:
GameMap.height
GameMap.width
```

- Это название слишком похоже на название стандартного интерфейса `Supplier` и к тому же является глаголом. Название классов должны быть существительными
```
class Supply
```

- Название класса должно быть существительным
```
class EntityInitCreation

//ПРАВИЛЬНО:
class EntitySpawner
```

*Oracle Java code conventions, part."Naming conventions"*  
*Мартин, "Чистый код", гл.2*  
*Ютуб, Немчинский "Как называть переменные, методы и классы?"*

**2. Если в блоке if есть return**(break, continue, throw, exit и т.д.), то else не пишется.  
В этом случае неважно, будет else или нет, так как программа будет работать одинаково и код без `else` будет выглядеть читабельней
```
if (firstDiff == secondDiff) {
 return (o1.equals(o2))
  ? 0
  : 1;
} else {
  return -1;
}

//ПРАВИЛЬНО:
if (firstDiff == secondDiff) {
 return (o1.equals(o2))
  ? 0
  : 1;
} 
return -1;
```

**3. Нарушение DRY**, магические буквы, числа, слова. Вводи константый
```
public void startInfinitySimulation() {
  //...
  gameOutput.showMessage("Введите p что бы остановить бесконечную симуляцию: \n");
}

class Pause implements Runnable {
  //...
  if (!(Character.toLowerCase(userEntered) == 'p')) 
}

//ПРАВИЛЬНО:
private final static char PAUSE = 'p';

public void startInfinitySimulation() {
  //...
  String message = "Введите '%c' что бы остановить бесконечную симуляцию: \n".formatted(PAUSE);
  gameOutput.showMessage(message);
}

class Pause implements Runnable {
  //...
  if (!(Character.toLowerCase(userEntered) == PAUSE)) 
}
```
*Фаулер, "Рефакторинг", гл.8, "Замена магического числа символической константой"*  
*refactoring.guru "Замена магического числа символьной константой"*  

**4. record Cell(int row, int column)**

+ 👍 Нет ничего лишнего, это хорошо. Record для координаты- идеально.

+ 👍 Хорошо, что поля координаты называются row и column а не x и y. Мне так больше нравится.  
В этом случае нет, как часто бывает, путаницы с порядком расположения в массивах: координата(x,y), а в массивах наоборот- массив[y, x].

**5. class GameMap**

- Используется две хешмапы- для хранения координаты-существа и существа-координаты
```
private final Map<Cell, Entity> cellEntityGameMap = new HashMap<>();
private final Map<Entity, Cell> entityCellGameMap = new HashMap<>();
```
В принципе, почему бы и нет- это делает алгоритм более громоздким, но некоторые операции с картой будут выполняться быстрее.

- Нарушение SRP, методы чужих ответственностей. 

Карта должна только хранить существа и обеспечить базовые операции с ними: вставить, выдать одно существо и список всех хранимых существ, удалить.
Если какой-то метод не нужен для обеспечения этого функционала, значит он не принадлежит к ответственности карты, а принадлежит к чужой ответственности.
Здесь методы чужих ответственностей: рассчитать площадь карты и найти соседние ячейки.

Наверное, для проекта в целом полезно иметь метод, который ищет соседние ячейки. 
Но этот процесс не имеет никакого отношения к единой ответственности карты- хранению существ в себе.  
Для организации хранения/удаления/выдачи существ, карте не нужно уметь искать соседние ячейки.  

Методы чужих ответственностей должны находиться в тех классах, в интересах которых они работают.
Если один и тот же метод используют разные классы, то метод нужно вынести в отдельный класс, например, BoardUtils.

- При всех операциях с участием координаты(добавить, выдать, удалить и т.д.), нужно проверять координату на корректность. 
Если координата некорректна(находится вне пределов карты), нужно бросать исключение
```
private boolean isAtBorder(Cell cell) {
  if (cell.row() >= 1 && cell.row() <= mapHeight) {
    if (cell.column() >= 1 && cell.column() <= mapWidth) {
      return true;
    }
  }
  return false;
}
```
Сейчас, если у карты спросить, свободна ли ячейка с координатой (+100500, -100500), то карта скажет, что свободна. 
А правильный ответ- такой координаты в карте вообще нет.

- Координаты в карте должны начинаться с точки 0,0 а не 1,1
```
if (cell.row() >= 1 && cell.row() <= mapHeight)
```

Википедия:
```
В декартовой системе координат, начало координат — это точка, в которой пересекаются все оси координат. 
Это означает, что все координаты этой точки равны нулю. 
Например, на плоскости она имеет координаты (0,0)
```

+ 👍 Хорошо, что возвращается копия мапы, а не оригинал. 
Тем самым соблюдается инкапсуляция и защищается внутреннее устройство карты
```
public Map<Cell, Entity> getCellEntityGameMap() {
  return new HashMap<>(cellEntityGameMap);
}
```

- Метод совершения хода в карте- нарушение SRP.
Теперь можно вызвать из карты метод хода и телепортировать зайца из одного края в другой минуя все правила игровой логистики
```
Карта карта = new Карта(100, 100);
карта.putEntity(new Coordinates(0, 0), new Заяц());
карта.moveEntity(new Coordinates(0, 0), new Coordinates(99, 99));

/* class Карта */
public void moveEntity(Cell from, Cell to) {
  if (!cellEntityGameMap.containsKey(from) || cellEntityGameMap.containsKey(to)) {
    throw new IllegalArgumentException("Incorrect entity");
  }
  Entity entity = getEntity(from);
  removeEntity(entity);
  placeEntity(to, entity);
}
```
Должна ли карта учитывать логистику зайцев? Если да, то как? 

Здесь этот вопрос частично решается- выполняется некоторая проверка при ходьбе. 
Значит, карта берет на себя часть ответственности по перемещению существа- карта знает *часть* правил корректного передвижения существ.
Как следствие, правила движения находятся в нескольких классах одновременно, в том числе и здесь.

Карта не должна знать этих правил, они должны находиться в одном месте- классе, который совершает передвижение. 
А значит, Карта не должна иметь метод телепортации.

**6. class GameMapUtils**

+ 👍 Здесь находятся вспомогательные операции для работы с картой: найти пустые координаты, посчитать количесвто существ определенного класса etc. 
Это хорошо.

- Утилитный класс это класс со статическими методами. Здесь методы не статические, а значит, это не утилита, а "Manager".

**7. interface PathFinder**

+ 👍 Интерфейс поиска пути это хорошо. Теперь можно делать разные реализации поиска: BFS, AStar и т.д.

- Нарушение SRP.

Поиск должен просто искать путь от точки старта до точки, соответствующей заданным условиям. 
В данном случае- до точки, в которой находится существо нужного класса.

Если поиск получает во входящие координаты цели, значит часть работы поиска уже выполнил кто-то другой. 
То есть, кто-то другой уже нашел цель и передал ее кординату в поиск пути
```
List<Cell> getPath(Cell target);
```
На самом деле поиск пути должен сам искать цель и прокладывать к ней путь. Сигнатура метода для этого должна выглядеть примерно так
```
List<Cell> getPath(GameMap gameMap, Cell start, Class<? extends Entity> target); {
  //ищет путь на карте от точки start
  //до точки, где находится существо нужного класса(напр. target == Grass.class)
}
```

**8. class Bfs implements PathFinder**, BFS-реализация интерфейса поиска пути

+ 👍 Хорошо, что класс называется "Bfs", а не "BFS". Среди прочего, это в тексте не будет путать с константой.

- Лишние зависимости
```
public Bfs(Creature creature, GameMap gameMap, GameMapUtils gameMapUtils) {
  //...
  this.creaturePosition = gameMap.getCell(creature);
}

//ПРАВИЛЬНО:
public Bfs(Cell creaturePosition, GameMap gameMap, GameMapUtils gameMapUtils) {
  //...
  this.creaturePosition = creaturePosition;
}
```

- Для BFS поиска не нужно знать координату цели. 
Этот поиск ищет первую попавшуюся ячейку с подходящим существом, а не прокладывает путь к какой-то конкретной ячейке
```
public List<Cell> getPath(Cell target)
```

- Мапа координата-координата обычно при реализации поиска используется как эрзац-заменитель связного списка
```
Map<Cell, Cell> cellParent = new HashMap<>();
```
Вместо мапы координата-координата сделай класс связного списка, алгоритм тогда будет выглядеть понятнее.
Например, так:
```
class Node {
  private Coordinate coordinate; 
  private Node parent;
  //...
  public Coordinate getCoordinate() {...}
  public Node getParent() {...}
}
```

**9. abstract class Entity и простые потомки Rock/Tree**

👍 Идеально
```
public abstract class Entity {
}
```
public class Rock extends TerrainObject{
}

**10. class Grass extends Supply**

Нарушение ООП дизайна в наследовании
```
public Grass() {
  super();
  this.supplyHeal = DEFAULT_GRASS_SUPPLY_HEAL;
}

//ПРАВИЛЬНО:
public Grass(){
  super(DEFAULT_GRASS_SUPPLY_HEAL);
}
```

**11. enum EntityType**

- Енам существует для создания системы дополнительной внешней типизации классов
```
enum EntityType {
  CREATURE(Creature.class),
  HERBIVORE(Herbivore.class),
  //...
}
```
Не нужно использовать дополнительную типизацию. 
Идентифицировать объекты на их принадлежность к классам нужно через стандартные способы: метод `getClass()`.

**12. abstract class Creature extends Entity**

+ 👍 Благодаря тому, что еда хранится в листе, для каждого вида существ можно хранить несколько видов еды.

- Не используй систему дополнительной типизации классов
```
protected final List<EntityType> targetEntities;

//ПРАВИЛЬНО:
protected final List<Class<? extends Entity>> targetEntities;
```

- Нарушение SRP.

Судя по сигнатуре, клиентский код уже уже выполнил часть работы- нашел путь, по которому должна пройти креатура.
Самой креатуре предлагается довериться этому пути
```
void makeMove(List<Cell> path, GameMap gameMap)
```
Если существо в себе содержит алгоритм передвижения, то и путь передвижения оно должно тоже находить самостоятельно.

**13. class Predator/Herbivore extends Creature**

Снова неправильная инициализация данных при наследовании
```
private int damage;

public Predator() {
  this.healthPoints = DEFAULT_PREDATOR_HEALTH_POINTS;
  this.damage = DEFAULT_PREDATOR_DAMAGE;
  this.stepLength = DEFAULT_PREDATOR_STEP_LENGTH;
  targetEntities.add(EntityType.HERBIVORE);
}

//ПРАВИЛЬНО:
private int damage;

public Predator() {
  super(
    DEFAULT_PREDATOR_HEALTH_POINTS,
    DEFAULT_PREDATOR_STEP_LENGTH,
    List.of(EntityType.HERBIVORE) // List.of(Herbivore.class)
  )  
  this.damage = DEFAULT_PREDATOR_DAMAGE;
}
```

**14. abstract class Action**

Не всем потомкам Action нужна фабрика существ, поэтому эта фабрика не должна здесь хранится. 
Она должна появиться на том уровне наследования, где будет использоваться
```
public abstract class Action {
  protected final EntityFactory entityFactory;
  //...
}
```

**15. class EntityInitCreation extends InitAction**

Каждый раз  при вызове методов происходит создание списка ВСЕХ возможных координат. Алгоритм явно избыточен и нуждается в пересмотре
```
public void execute() {
  fillCellsList(cells);
  //...
}

private void fillCellsList(List<Cell> cells) {
  for (int row = 1; row <= gameMap.getMapHeight(); row++) {
    for (int column = 1; column <= gameMap.getMapWidth(); column++) {
      Cell cell = new Cell(row, column);
      cells.add(cell);
    }
  }
}
```
Для начала можно создать такой сисок только один раз в самом начале, а дальше просто клонировать его на каждом ходе:
```
List<Cell> temp = new ArrayList<>(cells);
```
А потом нужно вообще разобраться с целесообразностью такого алгоритма. 

**16. class MoveCreaturesAction extends TurnAction**

Куча логики по передвижению существ.

Сейчас логика, которая выполняет передвижение существ, разделена на три класса: этот, Creature `makeMove(...)` и GameMap `moveEntity(...)`.

Эта логика должна лежать либо в самой креатуре и быть реализована в его методе `makeMove(...)`.  
И тогда креатура интерпритируется как сущность с собственной волей.

Либо находиться в отдельном классе, например `MoveCreaturesAction`, но тогда в самой креатуре не должно быть `makeMove(...)`.
И сущность будет интерпритироваться как сущность без собственной воли, как фигура в шахматах, которую передвигает кто-то еще.

Если логика передвижения находится в Creature, то мувер должен только инициализировать движение.  
Например, так:
```
class MoveAction реализует Action {

  @Override
  public void execute(Карта карта) {
    List<Creature> creatures = getCreatures(карта);
    for(Creature creature: creatures) {
      creature.makeMove(карта);  //даёт пинка
    }
  }

  private static List<Creature> getCreatures(Карта карта) {
    //найти и вернуть все креатуры из карты
  }
}
```

**17. interface UserInput**

Лучше сделай интерфейс универсального инпутера и его реализации для конкретного типа данных 
```
public interface UserInput {
  int getInteger() throws IOException;
  char getChar() throws IOException;
}

//ЛУЧШЕ:
public interface UserInput<T> {
  T get();
}

public class IntUserInput implements UserInput<Integer>  {
  @Override
  public Integer get() {...}
}

public class CharUserInput implements UserInput<Character>  {
  @Override
  public Character get() {...}
}
```
Еще лучше- использовать [ДИАЛОГИ](https://github.com/raketareview/simulation_review/blob/master/content/rev-sim076-ivan-LlqWst.md)

**18. class ConsoleInput implements UserInput**

Свои пользовательские исключения делай непроверяемыми(unchecked)

Тогда среди прочего не придется усложнять сигнатуры методов
```
public int getInteger() throws IOException {
  //...
  if (!userInput.matches("^[0-9]+$")) {
    throw new IOException("Incorrect input by user, user entered: " + userInput);
  }
}

//ПРАВИЛЬНО:
public int getInteger()  {  <-- НЕТ МУСОРА В СИГНАТУРЕ
  //...
  if (!userInput.matches("^[0-9]+$")) {
    throw new IllegalArgumentException("Incorrect input by user, user entered: " + userInput);
  }
}
```
*"ЧК", гл.7, "...проверяемые исключения"*

- Нарушение DRY
```
public int getInteger() throws IOException {
  //...  
  throw new IOException("Incorrect input by user, user entered: " + userInput);
}

public char getChar() throws IOException {
  //...
  throw new IOException("Incorrect input by user, user entered: " + userInput);
}
```

**19. interface GameOutput и его консольная реализация class ConsoleOutput implements GameOutput**

+ 👍 Вместе с `interface UserInput` это создает фактически слой `View`, который отделяет логику от представления. Это хорошо
```
public interface GameOutput {
    String ASK_FOR_CHOOSE_MESSAGE = "Сделайте свой выбор: ";
    String INCORRECT_USER_INPUT_MESSAGE = "Неверный выбор.";
    void showMessage(String message);
}
```

- *Эти* константы не должны быть в этом интерфейсе
```
String ASK_FOR_CHOOSE_MESSAGE = "Сделайте свой выбор: ";
String INCORRECT_USER_INPUT_MESSAGE = "Неверный выбор.";
```
Потому что эти константы- фразы на конкретном языке. 
Следовательно, этот интерфейс теперь зависит от представления- в данном случае от конкретного языка.
Эти константы нужно перенести в какое-то другое место.

**20. Меню**

+ 👍 Меню в ООП стиле 😎🤝😎

+ 👍 Правильно, что печать идет не напрямую, а через слой Вью
```
gameOutput.showMessage(item);
```

**21. interface Renderer**

👍 Интерфейс рендерера это хорошо. Теперь можно делать разные рендереры для разных визульных сред(консоль, интерфейс виндовс, http, матричный принтер etc)
и разного отображения информации(цветной, черно-белый и пр.)
```
public interface Renderer {
  void render(GameMap gameMap);
}
```

**22. class ConsoleRenderer implements Renderer**

- Нужно более информативное сообщение
```
switch (entity.getClass().getSimpleName()) {
  //...
  default -> {
    throw new IllegalArgumentException("Unclassified entity");
  }
}

//ПРАВИЛЬНО:
switch (entity.getClass().getSimpleName()) {
  //...
  default -> throw new IllegalArgumentException("Unclassified entity: " + entity);
}
```

- Печатать инфу нужно тоже через слой Вью. Так же, как это сделано в меню:
```
System.out.println(mapState);

//ПРАВИЛЬНО:
gameOutput.showMessage(mapState);
```

**23. class SimulationInitialization**

Стандартным явлениям давай стандартные названия. 
По функционалу это- фабрика класса Simulation, то есть SimulationFactory
```
public class SimulationInitialization {
  //...
  public Simulation init() {...}
}

//ПРАВИЛЬНО:
public class SimulationFactory {
  //...
  public Simulation get() {...}
}
```

**24. class Simulation implements MainMenuHandler**

👍 Принимает достаточное количество зависимостей в конструктор
```
public Simulation(GameOutput gameOutput, UserInput userInput, GameMap gameMap, EntityFactory entityFactory, GameMapUtils gameMapUtils)
```

**25. class Main**, содержит точку входа main

👍 Только создает и запускает симуляцию, в данном случае через меню, это хорошо.

## ВЫВОД

В целом, норм 👍

n.108(246)  
#ревью #симуляция 