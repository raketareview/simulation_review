...
- Пакеты называй или в множественном числе или в одиночном
```
actions
event
...
Замечание дискуссионное, зелёное по [классификации Фёдора](https://gist.github.com/losevskiyfz/69979db2bb6026ebc548366fbc573d5f). 
...
+ 👍 Класс содержит только необходимые для выполнения единой ответственности методы.

**5. class EntityLocator**

Утилитный класс, который содержит вспомогательные методы для работы с картой. 
Например, поиск соседних ячеек вокруг координаты. Притензий к классу нет.  
...
```
Существующий подход потенциально багоопасен, потому что теоретически креатуре можно в метод совершения хода отправить не тот класс существа, которое оно ест. 
...
**17. Создание экземпляра через классы Main и SimulationConfigurator**

Что `Main` класс большой и сложный, что `SimulationConfigurator`, что их взаимоотношения- тоже сложные.  
...
Action передвижения существ передвигает существ по карте, как игрок шахматные фигуры по доске. 
...
Так вот, при создании MoveAction в классе Simulation, к нему на телефонную трубку будет привязан метод распечатки информации о совершенном ходе
```
...
Во-вторых, через 2-3 недели посмотри на проект свежим взглядом. Если для тебя тогда все так же будет понятно, как работают евенты и экшены, значит с ними все ок и мои притензии к ним необоснованы.  
А если ты сам не сможешь понять, как это все работает, то можешь отрефакторить и сделать сделать проект более простым и понятным.

Другие, более простые замечания, при желании можешь отрефакторить сейчас, до перехода к новому проекту.  
В любом случае, практика с паттернами тебе пошла в плюс.
