...
private final static int EXCEPTION_MESSAGE = "Size must be between %d and %d.".formatted(MIN_WORLD_SIZE, MAX_WORLD_SIZE);
...
Видимо, ты геттеры и сеттеры для всех классов делаешь автогенерацией "Generate -> Getter and Setter" просто на всякий случай.  
...
Класс не должен реализовывать в себе самостоятельный поиск цели, либо частично его реализовывать.
...
public abstract class Creature extends Entity {
  ...
  public abstract void makeMove(BreadthFirstSearch pathfinder, WorldMap worldMap) {
    ...
    List<Coordinates> path = pathfinder.findPath(worldMap, myCoordinates, en -> isTarget(en));  //findPath(..., Predicate<Entity> targetDetector)
    //пройти по пути path
  }
}
```
...
Действия выполняются над картой. Можно карту принимать в конструктор- так, как ты это делвешь:
```java
public interface Action {
  void execute();
}
```
Можно карту принимать в аргументы метода:
```java
public interface Action {
...
👍 Всё ок. Всё ок, только инициирует движение креатур(даёт им пинка- дальше они сами бегут).
