...
- В яве всё- объекты. Конкретизируй, какие именно объекты ты имеешь ввиду
```java
...
Например, есть игровая доска 100*100 и в ней одномоментно находится общим числом 150 разных существ: камней, травы, зайцев и т.д.
...
for(Coordinate shift : SHIFT_COORDINATES) {
  Coordinate next = new Coordinate(shift.x() + current.x(), shift.y() + current.y());
  if(isInside(next)) {
    neighbors.add(next);
  }
}
...
Если на уровне `Entity` нужно сообщать свой спрайт(это неправильно, но если), 
...
Точку входа `main()` нужно вынести в отдельный класс, который будет только собирать все зависимости и запускать программу.  
...
Поэтому, согласно паттерна Creator, объект `ConsoleRenderer` *здесь* должен создавать сам класс `Ecosystem`.
