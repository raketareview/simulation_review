...
private int countHerbivore;  // = 0
```
Поля класса-объекты по умолчанию инициализируются значением null, тип int инициализируется значением 0 и т.д.

**4. record Coordinate(int x, int y)**

+ 👍 Нет ничего лишнего, это хорошо. Record для координаты- идеально.

- Координата это принадлежность пакета игровой карты, а не entity.  
...
```  
Для начальной инициализации карты специально придуманы конструкторы. 

Использование вместо коснтрукторов инициализационных метидов приводит к тому, что правила использования такого класса зпметно усложняются.
...
```
Пусть не смущает слово "default"- создать карту можно только с фиксированными размерами, а не на выдор с дефолтными и кастомными.
...
И выполняет миллион других дел, которые не имеюют отношения к единой ответственности карты.
...
+ 👍 Хорошо, что возвращается копия, а не оригинал хэшмапы. Тем самым соблюдается инкапсуляция какрты
```
...
Самой карте не нужно знать количество хранимых зайцев- это никак не влияеет на ее действия по вставить/вернуть/удалить существа.

Нарушение же открытости/закрытости состоит в том, что при добавлении в проект новых видов существ, которые придется считать, 
это приведет к изменению класса карты: `void counterGrassAndHerbivoreAndBird()`.
...
```
List<Coordinate> findPath(Coordinate start, Coordinate target, MapEntity map, Class<? extends Entity> type);
```
...
+ 👍 Направление в виде константы-списка координат смещения это хорошо
```
...
Поэтому единственная операция, которую поиск должен проводить с классом существа- это определить, подходит ли оно по условию поиска.
Примерно так:
```
List<Coordinate> findPath(MapEntity map, Coordinate start, Class<? extends Entity> target) {
  //...
...
В этом классе должно происхлдить создание и заселение существ на карту, а не делегирование этой функции самой карте
```
@Override
public void perform(MapEntity map) {
...
То же самое качается стартовых экшенов:
```
...
Потому в коде регулярно нарушаются простейшие правила: методы называются существительными, а поля и классы- глаголами.
...
```
В твоей реализации цикл симуляции вполне конечный и довольно короткий- до 100:
```
...
Action'ы, в целом, сделаны нормально. Но используются неправильно- изучить паттерн "Команда".
