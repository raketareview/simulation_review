...
- Пакет `service` содержит классы, которые нельзя отнести к сервисам, например, `Renderer`, `BFS` etc. Очевидно, что под "сервисом" здесь понимается просто класс, который просто что-то как-то делает.  
