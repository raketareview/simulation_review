...
- Возврат констант- не повод нарушать конвенцию наименований (даже если бы метод возвращал константу)
```java
public int getHP() {
  return HP;
}
...
Придется изменить класс. То есть, распечатка меню здесь это лишняя причина для изменения класса- нарушения SRP.
