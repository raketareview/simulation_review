https://github.com/aleksLiss/simulation_app  
[Леха 47]

Есть над чем поработать.

## НЕДОСТАТКИ РЕАЛИЗАЦИИ

1. Карта распечатывается неровно, нужно подобрать спрайты одинаковой ширины  
![pic](https://github.com/raketareview/simulation_review/blob/master/content/resources/rev-sim113/img0.png) 

2. Нет паузы/пуск во время работы симуляции, хотя это требование есть в ТЗ.

## ХОРОШО

+ 👍 Координаты существ не хранятся в самих существах(мне так больше нравится)

## ЗАМЕЧАНИЯ

**1. Нейминг**

- Не сокращай без необходимости, тем более имя класса
```
class Coords

//ПРАВИЛЬНО:
class Coordinates
```

- Для избежания путаницы не называй пакеты, классы, интерфейсы так же, как называются классы и интерфейсы стандартной библиотеки.
Есть стандартные интерфейсы `Set` и `Map`, не должно быть кастомных пакетов с таким же названием
```
package ru.simulation.app.action.set;
package ru.simulation.app.map;
```

- Если в проекте есть класс `SimulationMap`, то все переменные с именем, включающим это название, должны быть экземплярами этого класса.
Если разные концепции называются одним и тем же именем, это приводит к путанице
```
public class SimulationMap {
  private Map<Coords, Entity> simulationMap = new HashMap<>();
  //...
}

//ПРАВИЛЬНО:
public class SimulationMap {
  private Map<Coords, Entity> entities = new HashMap<>();
  //...
}
```
Потому что иначе будет непонятно, являются ли эти две переменными одного типа:
```
var simulationMap = new SimulationMap(10);
var simulationMap0 = simulationMap.getSimulationMap();
```

То же самое:
```
public class Icon {
  private String icon;
  //...
}
```

- Избыточное пояснение. И так понятно, что раз мы находимся в Карте, то и действия происходят в рамках этой Карты
```
public class SimulationMap {
  public void putIntoMap(Coords coords, Entity entity)
  public Entity getEntityFromMap(Coords coords)
  public void deleteFromMap(Coords coordinate)
  //...
}

//ПРАВИЛЬНО:
public class SimulationMap {
  public void put(Coords coords, Entity entity)
  public Entity get(Coords coords)
  public void delete(Coords coordinate)
  //...
}
```

- Непонятные названия

Концепция "ключ" относится к подробности внутреннего устройства класса  SimulationMap.
Там используется хэшмапа, которая хранит записи ключ-значение
```
public class SimulationMap {
  private Map<Coords, Entity> simulationMap = new HashMap<>();

  public boolean isContainsKey(Coords coords) {
    return simulationMap.containsKey(coords);
  }
  //...
}

//ПРАВИЛЬНО:
public boolean isBusyCoordinates( Coords coords) {...}
```
Клиент не обязан знать внутреннее устройство используемого класса. 
Клиент должен знать только те абстракции, которые описывают его взаимодействие с этим классом.

- Не нужно в названии метода пересказывать его сигнатуру
```
List<Coords> getGrassesCoordsList()

//ПРАВИЛЬНО:
List<Coords> getGrassesCoordinates()
```

- Для избежания недоразумений, не используй в назывании переменных слово "Object", если они там не хранят объекты именно класса `Object`.  
Да, все классы являются наследниками `Object`. 
Именно поэтому так назвать переменную- все равно, что никак её не назвать
```
List<Coords> staticObjects

//ПРАВИЛЬНО:
List<Coords> staticEntities
```

- Название класса должно быть существительным
```
class CalculatePath
class AddGrass

//ПРАВИЛЬНО:
class PathFinder
class GrassSpawnAction
```

- Придерживайся единообразия. Названия пакетов делай либо в единственном, либо во множественном числе
```
model
utils
```

- Это не случайная координата, это класс для создания случайной координаты
```
public class RandomCoordinates {
  public static Coords getRandomCoordinate(int size) {...}
}

//ПРАВИЛЬНО:
public class RandomCoordinatesFactory {
  public static Coords get(int max) {...}
}
```

- Не дублируй имя класса в названии его метода
```
public interface Action {
  void doAction();
}

//ПРАВИЛЬНО:
public interface Action {
  void execute();
}
```

*Oracle Java code conventions, part."Naming conventions"*  
*Мартин, "Чистый код", гл.2*  
*Ютуб, Немчинский "Как называть переменные, методы и классы?"*

**2. Кривое форматирование**

Приоритет- читаемость кода. Не стремись засунуть несколько инструкций в одну.  
Особенно, когда эта инструкция получается такой длинной, что ее хочется написать в несколько строк
```
simulationMap.putIntoMap(
  RandomCoordinates.getRandomCoordinate(simulationMap.getSize()), new Predator());

//ПРАВИЛЬНО:
Coords coords = RandomCoordinates.getRandomCoordinate(simulationMap.getSize())
simulationMap.putIntoMap(coords, new Predator());
```

**3. Разбиение на пакеты**

- Пакет model

Судя по названию, в этом пакете должны находиться модели.  
Следовательно, остальные классы проекта *не должны* являться моделями.  
Но не все классы-модели проекта сейчас находятся в пакете "model".

Например, эти классы-модели находятся в других пакетах: SimulationMap, Coords, Counter.

- Пакет utils

Не все классы в пакете утилитные.  
Утилитные классы должны быть final, иметь закрытый конструктор, все их методы должны быть статическими.  
*Блох, "Java. Эффективное программирование" гл.2.4*

**4. class Coords**

+ 👍 Нет ничего лишнего, это хорошо. 

- Неиммутабельный класс

Объект является изменяемым - в классе есть сеттеры.  
Предпочтительнее этот класс сделать иммутабельным.

- Класс может быть преобразован в record без потери функционала. Но при этом он станет иммутабельным.

**5. class SimulationMap**

- Нет оснований ограничивать карту только квадратом
```
private int size;

//ПРАВИЛЬНО:
private int width;
private int height;
```

- Нет необходимости изменять размер карты после ее создания
```
public void setSize(int size)
```

- Нарушение SRP, OCP 

Карта должна работать со всеми хранимыми существами одинаково и не работать как-то по-особому с конкретным классом представителей и не должна даже знать по именам наследников Entity
```
public List<Coords> getHerbivoresCoordsList() {...}
public List<Coords> getPredatorsCoordsList() {...}
public List<Coords> getGrassesCoordsList() {...}

//ПРАВИЛЬНО:
public List<Coords> getAllCoordinatesBy(Class<? extends Entity> clazz)  //вернуть координаты существ определенного класса 
```

Здесь тоже какая-то логика, рассчитанная на знание конкретных типов существ и особенностей взаимодествия с ними:
```
private boolean isCellNotAccessible(Coords coords) {
  return (simulationMap.get(coords).getName() == 'r')
      || (simulationMap.get(coords).getName() == 't');
}
```

А здесь Карта знает не только про конкретные типы существ, но и про то, что некоторые из них объеденены в особые группы
```
public List<Coords> getStaticObjects()
```

- При всех операциях с участием координаты(добавить, выдать, удалить и т.д.), нужно проверять координату на корректность. 
Если координата некорректна(находится вне пределов карты), нужно бросать исключение
```
public boolean isContainsKey(Coords coords) {
  return simulationMap.containsKey(coords);
}
```
Сейчас, если у карты спросить, занята ли ячейка с координатой (+100500, -100500), то карта скажет, что свободна. 
А правильный ответ- ячейки с такой координатой в карте нет вообще.

Ближайшая аналогия- стандартные хранилища типа List и массива. 
При попытке обратитья к ним по несуществующему индексу, бросается исключение.


- Никогда не возвращай null
```
private Map<Coords, Entity> simulationMap = new HashMap<>();

public Entity getEntityFromMap(Coords coords) {
  return simulationMap.get(coords);  <-- может вернуть null
}
```
Возврат null повышает риск возникновения NullPointerException в программе.  
*Мартин, "Чистый код", гл.7.7-8*  
*Ютуб, Немчинский "Почему нельзя возвращать NULL?"*

**6. class CalculatePath**

- Нарушение конвенции кода. Главный публичный метод, которым пользуется клиент, должен стоять вверху. Сейчас он стоит в самом низу
```
public static Coords getNextCoordinate(List<Coords> exclusion, Coords start, Coords finish)
```

- Нарушение принципа минимально открытого интерфейса.

В классе поиска пути должен быть только один публичный метод- тот, который ищет путь.  
Клиент должен видеть только те методы, которые нужны для выполнения поставленной задачи.  
*Вайсфельд "Объектно-ориентированный подход", гл.5, "Минимально открытый интерфейс"*

- Сейчас класс не ищет путь

Поиск должен найти и вернуть путь к цели. Путь это последовательность координат. 

Сейчас класс ищет не путь к цели, а следующую точку к цели. 
Но чтобы ее получить, нужно самостоятельно заранее рассчитать какие-то вспомогательные координаты
```
public static Coords getNextCoordinate(List<Coords> exclusion, Coords start, Coords finish)
```

Поиск должен просто искать путь от точки старта до точки, соответствующей заданным условиям. 
В данном случае- до точки, в которой находится существо нужного класса.

Если поиск получает во входящие аргументы метода координаты цели, значит часть работы поиска уже выполнил кто-то другой. 
То есть, кто-то другой уже нашел цель и передал ее кординату в поиск пути.

На самом деле поиск пути должен сам искать цель и прокладывать к ней путь. 
Сигнатура метода для этого должна выглядеть примерно так
```
List<Coordinate> findPath(Карта карта, Coordinate start, Class<? extends Entity> target) {
  //ищет путь на карте от точки start
  //до точки, где находится существо нужного класса(напр. target == Grass.class)
}
```

- Магические цифры
```
return (Math.min(diffX, diffY) * 14) + (Math.abs(diffX - diffY) * 10);
```

**7. class Icon**

Класс не делает вообще ничего, он не нужен. 
Это просто обертка над строкой
```
public class Icon {
  private String icon;

  public Icon(String icon) {
    this.icon = icon;
  }

  public String getIcon() {
    return icon;
  }

  public void setIcon(String icon) {
    this.icon = icon;
  }
}
```

**8. abstract class Entity**

- Дополнительная идентификация класса через специальное поле
```
public char name;
```
Поле `name` здесь используется для определения принадлежности к определенному типу(фактически классу) существ. 

Не нужно в ентити хранить специальное поле для дополнительной типизации классов.

Для определения принадлежности к классам, нужно использовать стандартные методы
```
return (simulationMap.get(coords).getName() == 'r')
    || (simulationMap.get(coords).getName() == 't');

//ПРАВИЛЬНО:
return (simulationMap.get(coords).getClass() == Rock.class)
    || (simulationMap.get(coords).getClass() == Tree.class);
```

- Нарушение SRP, зависимость модели от представления

Существо хранит спрайт с собственным изображением в виде объекта специального класса-обертки
```
public Icon icon;
```
Модель(а это модель) не должна зависеть от представления и знать, как ее будут показывать юзеру.
Потому что в разных средах(консоль, Swing, Android) одна и та же модель может быть показана разными способами.
Спрайты всех существ должны храниться в классе, который распечатывает карту.

**9. class Grass extends Entity**

Нарушение дизайна ООП в наследовании. 
Данные должны передаваться в конструктор предка, а не сетиться
```
public class Grass extends Entity {
  public Grass() {
    this.icon = new Icon("\uD83C\uDF3F");
    this.name = 'g';
  }
}

//ПРАВИЛЬНО:
public class Grass extends Entity {
  private final static char NAME = 'g';
  private final static String SPRITE = "\uD83C\uDF3F";

  public Grass() {
    super(NAME, new Icon(SPRITE));
  }
}
```

**10. abstract class Creature extends Entity implements Move**

- Креатура не должна хранить в себе координаты еды
```
public abstract class Creature extends Entity implements Move {
  private Coords goal;
  //...
}
```
У Креатуры нет никакой необходимости хранить в себе координату еды. 
Во время совершения хода, креатура должна каждый раз запрашивать путь к еде и проходить по этому пути.
Хранить для этого координаты еды в поле класса нет никакой необходимости.

- Этот метод не выполняет движение, он возвращает какую-то координату.
Метод совершения хода должен совершать ход
```
public Coords makeMove(Coords start, Coords goal, List<Coords> exclusion)

//ПРАВИЛЬНО:
/*
* ищет путь до еды 
* проходит путь 
* ест еду
*/
public abstract void makeMove(Карта карта); 
```

**11. class Herbivore/Predator extends Creature**

- Нарушение дизайна ООП в наследовании. Дублирование данных в предке и в наследниках
```
public abstract class Creature extends Entity implements Move {
  private int hp;
  private int speed;
  //...
  
  public int getHp() {
    return hp;
  }

  public int getSpeed() {
    return speed;
  }
}

public class Predator extends Creature implements Move {
  private int hp;  <-- ДУБЛИРУЕТ ПОЛЕ ПРЕДКА
  private int speed;  <-- ДУБЛИРУЕТ ПОЛЕ ПРЕДКА

  public Predator() {
    this.hp = 100;
    this.speed = 2;
    //...
  }
  
  public int getHp() {
    return hp;  <-- ПЕРЕОПРЕДЕЛЯЕТ АНАЛОГИЧНЫЙ МЕТОД ПРЕДКА
  }

  public int getSpeed() {  <-- ПЕРЕОПРЕДЕЛЯЕТ АНАЛОГИЧНЫЙ МЕТОД ПРЕДКА
    return speed;
  }
}
//ТО ЖЕ САМОЕ У ЗАЙЦА
```

Общие поля и поведение выноси в предка:
```
public abstract class Creature extends Entity implements Move {
  private int hp;
  private int speed;
  //...
  public Creature(int hp, int speed, ...) {
    super(...);
    this.hp = hp;
    this.speed = speed;    
  }

  public int getHp() {
    return hp;
  }

  public int getSpeed() {
    return speed;
  }
}

public class Herbivore extends Creature {
  private final static int HP = 100;
  private final static int SPEED = 1;
  //...

  public Herbivore() {
    super(HP, SPEED, ...);
    //...
  }
}
```

**12. class Counter**

Нет оснований, чтобы этот класс был синглтоном. 
Вообще, синглетонизм сейчас считается антипаттерном
```
public class Counter {
 private static Counter instance;
 //...

  public static synchronized Counter getInstance() {...}
}
```

**13. class AddGrass/AddHerbivore/AddPredator implements Action**

Нарушение DRY.  
Несколько классов, которые делают одно и то же- заселяют в карту на случайные кооринаты существо определенного типа.  
Сделай универсальный класс, напрмер, такой:
```
public class SpawnAction implements Action {
  //...

  public SpawnAction(SimulationMap simulationMap, int number, Supplier<Entity> entitySupplier) {
    this.simulationMap = simulationMap;
    this.number = number;
    this.entitySupplier = entitySupplier;
  }

  @Override
  public void doAction() {
     for(int i = 0; i < number; i++) {
        Coords coods =  RandomCoordinates.getRandomCoordinate(simulationMap.getSize());
        Entity entity = entitySupplier.get();
        simulationMap.putIntoMap(coods, entity);
     }
  }
}
```
Использование:
```
SpawnAction rockSpawnAction = new SpawnAction(simulationMap, ROCK_NUMBER, ()-> new Rock());
SpawnAction treeSpawnAction = new SpawnAction(simulationMap, TREE_NUMBER, ()-> new Tree());

rockSpawnAction.doAction();
treeSpawnAction.doAction();
```

**14. Пакет set**

7 классов, которые во многом  дублируют друг друга.  

**15. class PullObjectsIntoMap implements Action**

Алгоритм заселения карты существами в количистве, зависимом от ее размера, очень громоздкий и примитивный:
```
int size = simulationMap.getSize();
if (size > 4 && size < 15) {
  setStaticObjects = new SetObjectsService(new SetObjectsForSmallMap(simulationMap));
}
if (size > 14 && size < 20) {
  setStaticObjects = new SetObjectsService(new SetObjectsForMiddleMap(simulationMap));
}
//и т.д. и т.п.
```

Лучше сделай класс, который будет заселять карту существами в процентах от общей площади карты.  
Типа такого:
```
class НачальноеЗаселениеAction implements Action {
  private final int rockNumber;
  private final int treeNumber;
  //...

  public НачальноеЗаселениеAction(
    Карта карта,
    int rockPercent, 
    int treePercent, 
    //...
  ) {
    this.rockNumber = //рассчитать количество камней от площади и rockPercent
    this.treeNumber = //рассчитать количество деревьев от площади и treePercent
    //...
  }

  @Override
  public void doAction() {
    //заселить камни на карту, количество камней: rockNumber
    //заселить деревья на карту, количество деревьев: treeNumber
    //заселить остальные существа в их количестве
  }
}
```

**16. class CheckCountObjects**

- Горы магических чисел.

- Нарушение DRY. 

Куча методов, которые делают одно и то же- ... делают что-то одинаковое в отношении конкретного вида существ:
```
public boolean isChangeCountGrass() {
  //опосредованное дублирование логики  
}

public boolean isChangeCountPredators() {
  //опосредованное дублирование логики  
}

public boolean isChangeCountHerbivore() {
  //опосредованное дублирование логики  
}
```

- Как и в `PullObjectsIntoMap`, какой-то громоздкий код в `setCountersObjects()`.

**17. class MoveCreatures implements Action**

Класс реализует часть логики передвижения существа. 
Другая часть логики реализуется в самом существе. 

Таким образом, единое действие, перемещение существа по карте, разделено на два класса.

Логика движения должна лежать либо в самой креатуре и быть реализована в его методе `makeMove(...)`.  
И тогда креатура интерпретируется как сущность с собственной волей.

Либо находиться в отдельном классе, например `MoveAction`, но тогда в самой креатуре не должно быть `makeMove(...)`.  
И сущность будет интерпретироваться как сущность без собственной воли, как фигура в шахматах, которую передвигает кто-то еще.

**18. interface Renderer**

👍 Интерфейс рендерера это хорошо.  
Теперь можно делать разные рендереры для разных визуальных сред(консоль, интерфейс виндовс, http, матричный принтер etc)
и разного отображения информации(цветной, черно-белый и пр.)
```
public interface Renderer {
  void render();
}
```

**19. class MapRenderer implements Renderer**

Спрайты существ должны храниться здесь, а не браться из ентити.

**20. class Simulation**

- Нарушение DI. Класс не должен инициализировать сам себя. 
В данном случае класс инициализирует сам себя тем, что сам создает карту фиксированного размера, а не принимает ее в конструктор
```
public class Simulation {

  public Simulation() {
    simulationMap = new SimulationMap(10);
    //...
  }
}
```
Таким образом нельзя создать несколько игровых конфигураций с разными картами разных размеров.  
*Мартин, "ЧК", гл.11, "Отделение конструирования системы от ее использования"*

- Должно быть два списка экшенов: экшены на старте и экшены на каждом ходе

```
private List<Action> setActions;
private List<Action> turnActions;
private List<Action> conditionActions;

//ПРАВИЛЬНО:
private List<Action> initActions;
private List<Action> turnActions;
```

- Экшены не должны запускаться по условию
```
if (checkCountObjects.isChangeCountGrass()  <-- ЗАПУСК ЭКШЕНОВ ПО УСЛОВИЮ
   || checkCountObjects.isChangeCountHerbivore()
   || checkCountObjects.isChangeCountPredators()
   ) {
  conditionActions.forEach(Action::doAction);
}
``` 

Там весь прикол в том, что должна быть возможность просто добавить новый экшен в список экшенов. 
И программа после этого должна работать с новым функционалом без изменения остального кода.

В данном случае экшены-балансировщики популяции должны внутри себя содержать проверку на то, нужно дополнительно увеличить популяцию или нет
```
turnActions = List.of(
  new MoveCreatures(simulationMap)
);
conditionActions = List.of(
  new AddGrass(simulationMap),
  new AddHerbivore(simulationMap),
  new AddPredator(simulationMap)
);

//ПРАВИЛЬНО:
turnActions = List.of(
  new MoveCreatures(simulationMap)
  new AddGrass(simulationMap),
  new AddHerbivore(simulationMap),
  new AddPredator(simulationMap)
);
```

- Выполнение `initActions`(здесь это называется `setActions`) должно происходить не в методе совершения хода, а в конструкторе
```
public void nextTurn() {
  if (counter.getCount() == 0) {
    setActions.forEach(Action::doAction);
  }
  //...
}

//ПРАВИЛЬНО:
public Simulation(...) {
  //...
  setActions.forEach(Action::doAction);
}
```

**21. class Main**, содержит точку входа main

👍 Только создает и запускает Симуляцию, это хорошо.

## ВЫВОД

Разобраться с наследованием- как правильно инициализировать поля у предка.  
Доделать функционал по ТЗ. 

Для лучшего понимания декомпозиции ООП посмотреть ролики Сергея про шахматы.  
Посмотреть ролики Немчинского про SOLID.

n.113(258)  
#ревью #симуляция 