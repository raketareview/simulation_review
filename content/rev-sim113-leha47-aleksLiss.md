...
Следовательно, остальные классы классы проекта *не должны* являться моделями.  
...
А здесь Карта знает что есть не только про конкретные типы существ, но и про то, что некоторые из них объеденены в особые группы
```
...
Нарушение DRY.  
Несколько классов, которые делают одно и то же- заселяют в укарту на случайные кооринаты существо определенного типа.  
...
Алгоритм заселения карты существами в зависимости от ее размера очень громоздкий и примитивный:
```
...
class НачальноеЗаселениеAction implements Action {
  ...
  public НачальноеЗаселениеAction(
    ...
    this.rockNumber = //рассчитать количество камней от полщади и rockPercent
    this.treeNumber = //рассчитать количество деревьев от полщади и treePercent
    //...
  }

  @Override
  public void doAction() {
    //заселить rockNumber камней на карту
    //заселить treeNumber деревьев на карту
    //заселить остальных
  }
}
```
...
Куча методов, которые делают одно и то же- ... делают что-то одинаковое в отношении конкретного вида существ, короче
```
...
```
Эти экшены-балансировщики популяции должны внутри себя содержать проверку на то, нужно дополнительно поместить существ в карту или нет
```
turnActions = List.of(
