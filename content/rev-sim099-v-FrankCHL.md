https://github.com/FrankCHL/Simulation.git  
[v]
...
```
Если нужно обозначить в названии класса его принадлежность к "пользовательскому интерфейсу", используй в названии слова "Ui" или "View".
...
//ПРАВИЛЬНО:
public class Board {
  private final Map<Node, Entity> gameMap;
...
Тотально везде в пректе доступ к полям классов из клиентских классов происходит напрямую, а не через геттеры, сеттеры и другие публичные методы.
...
Просто сделай два класса: один для хранения координаты, второй- для связного спсика. В каждом конкретном случае использую один из этих классов.
...
```
Доступ к полям класса должен быть происходить токль через публичные методы класса: геттеры, сеттери и т.д.
...
- Но есть один моменьтик: класс печатает карту одновременно в консоль и в Windows окно.
...
Далее, нужно сделать две игровые коонфигурации.  
Одна должна запускать игру с консольным визуальным интерфейсом, а другаю- с виндовым.  
Примерно, так:
```
...
Не видно понимания ООП даже на самом базовом уровне: инкапсуляция, полиморфизм, наследование, зачем нужны конструкторы в классах и т.д.  
