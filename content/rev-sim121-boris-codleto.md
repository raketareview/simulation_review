...
System.out.println("Карта 10х10 " + LARGE);
...
**В принципе, на этом можно было остановить разбор проекта, потому что дальше ничего хорошего уже быть не может. Нужно переделывать.**

Потому что неправильно выбранный подход,- предпочтение утилит нормальным объектам,- может привести только к плохой и неправильной архитектуре в ООП программе.
...
Сейчас метод совершения хода содержит 20 строк логики, которые описывают правила логистики.  
...
Здесь *данное* нарушение Low Coupling возникает из-за нарушения SRP- выполнением класса чужой ответственности по перемещению существ на карте.
...
Сейчас `Actions` это два класса, просто контейнеры функций, сделанный в стиле процедурного программирования.  
...
Сначала переделай экшены в стиле паттерна Command, потом можно будет о чем-то разговарить по их содержанию.
...
public interface PathFinder {
  ...
  List<Coordinates> find(Coordinate start, MapEntity map, Class<? extends Entity> target);  
}
```
...
То есть такие классы, которые содержат статические публичные методы и статические *изменяемые* поля.

Все пользовательские классы ты на *этом этапе* должен создавать через `new` и тем самым работать с ними как с экземплярами классов, а не классами-утилитами.
