...
Это не не константы, это просто финальные поля 
```java
...
"Реальная фабрика" скорее озадачивает- почему она реальная, есть еще нереальная?  
...
Поэтому нельзя общее называть как его частное
```java
PathFinder bfs;
...
public class RealEntityFactory implements EntityFactory {  <-- ФАБРИКА
  ...
  private final Map<Class<? extends Entity>, Supplier<? extends Entity>> creators = Map.of(
    Herbivore.class, () -> new Herbivore(1, 3, 7), 
    Predator.class, () -> new Predator(2, 4, 9), 
    //...
  );
  //...
}
...
Условный пример неправильного использования интерфейса:
```java
...
Потому что для этого интерфейса можно сделать разные реализации.
...
Если в проекте есть интерфейс, то клиенты должны с ним работать не как с конкретной реализацией, а как с интерфейсом согласно SOLID-принципу DIP.

## ВЫВОД

Работа над ошибками пошла на пользу, устранены грубые ошибки архитектуры прошлой версии.  
Всегда приятно видеть такой прогресс благодаря своему скромному в нём участию, ты молодец.  
