...
находит заданное количество пустых позиций на карте.
...
То же самое касается `ConsoleMainMenuRenderer` и `ConsoleMapRenderer`- спрайты существ etc.  
...
public class RespawnAction implements SpawnAction {
  ...
  public boolean execute(GameMap gameMap) {
    for(Map.Entry<EntityType, Integer> entry : ENTITY_MIN_NUMBERS.entrySet() ) {
      ...
      if(minCount > current) {  //существ меньше минимального- восполнить количество существ
...
Эти интерфейсы не завязаны на консоль или любцю другую среду ввода-вывода, поэтому не должны лежать в пакете "console"
```java
...
public class CArcadeMashineMainMenu implements Menu  <-- Гипотетическая реализация меню для аркадного автомата на лампочках
```
...
Например, неясно зачем нужен это отдельный рендерер(плюс его интерфейс) с пунктами меню:
```java
public class ConsoleControlHintRenderer implements HintRenderer {
  private static final String CONTROLS_HINTS = """
      1. Приостановить симуляцию
...
👍 Только создает зависимости, инжекит их в Симуляцию и запускает ее. Это хорошо.
